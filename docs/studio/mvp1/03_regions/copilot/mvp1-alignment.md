...
- 约束(2026-07-02 R5-E 图标 + 开关,PM「找一个类似 m 的星座图标」「把 copilot 开关换成圆形 m 图标放在 canvas 上,点击展开面板做过渡动画,再点收起」「裸 icon + 主题色贴背景看不清」):身份标 = **仙后座(Cassiopeia)五星连线**自绘 SVG(`components/copilot/moirai-mark.tsx`,`currentColor` 主题化),一形三读——字母 **M**、**星座**、**节点 + 边的图(DAG)**。面板开关**从顶栏移到画布**:面板收起时画布右下角浮现**实心 primary 圆形 FAB**(`copilot-fab.tsx`,实心底 + 光晕,修「裸 icon 贴近黑画布看不清」),点击展开面板走**从右下角生长**的过渡动画(fade + zoom + slide,`origin-bottom-right`,~200ms;`prefers-reduced-motion` 退化为纯 fade,zoom/slide 仅 `motion-safe`);面板 header 的收起控件(`PanelRightClose`)缩回 FAB。顶栏原 `Sparkles` 开关删除。
