   ...
   - **修订记录（PM 2026-07-02，协议探测矩阵）**：#C 的"排列组合各测一遍"落地形态定为**探测矩阵**，并纠正实现 drift（"轮换选出唯一协议并改写 endpoint.protocol"）。原则：**状态不能被设置，只能从观察算出来；观察会老化，但不会被覆盖**。
     ...
     6. **routes 只挂在活格子上**：**格子身份搬家时清除其名下 routes**（第 3 点"改地址即作废旧观察"）：routes 就是"在那个地址问出来的模型清单"，地址换了它们描述的就是别处；清除后走与 endpoint 删除相同的级联，把 role 里指向这些 route 的引用一并摘掉，不留下解析不了的悬空引用。瞬时失败不清。
        ...
        - **为什么不顺手把名下 route 一起停掉（待办，非本次范围）**：论点是"死传输不该再收到 prompt"，
          ...
          旧 role 绑定会在真实调用上失败并沿 fallback 链继续 —— 这与**任何其它** `failed` endpoint
          今天的代价完全一样（它们同样不拦执行）。这个代价有界；而在一次误判上删掉用户的 role 绑定
          没有界。
...
- **一条链的判据看「有没有答上来的」,不看「最差的那条」**(2026-08-21 立,问题台账 L7)。
  上一条说的是**探**所有:一条都不少地问。这一条说的是**答**——把那些答案收成一句话时该怎么收。
  - **规则**:全部答上来 = `ok`;**有一条答上来、也有答不上来的 = `warning`**;一条都没答上来才是
    失败(其中有账号级 `blocked` 就报 `blocked`,那是读者能动手的那个原因);**一条都没有可测的路由
    不算通过**,按失败报——「没东西可测」和「每条都过了」不是同一件事。
  - **为什么**:fallback 链存在的理由就是**一条挂了还能从别的路走**。把链上任意一条失败读成
    「这条链不通」,等于用链的设计目的去否定链本身。实证(2026-08-21):一个 compare 候选选
    `Auto fallback` 按 Test,16 条路由里 **12 条答上来**(含它历史成功 run 一直在用的那条),
    界面报的却是 `Test failed`,而且只列出失败的 4 条——**一个完全能用的候选被报成坏的**,
    读者从屏幕上看不到任何一条通了的证据。
  - **判据必须一次看完整条链,不能边走边合并**:「一条失败要不要否定这条链」取决于**其余路由
    答没答上来**,而逐条累积的写法在看到第二条时已经把第一条忘了。所以聚合函数收的是**全部
    结果的列表**,不是「当前状态 + 下一条」。
  - **失败不因此消失**:`warning` 不是「没事」,它是「能用,但不是每条都通」——失败逐条仍然列在
    详情里,且照常回写 Probe Knowledge Catalog(上一条不变)。
  - **报告必须说出「多少条答上来了」**:详情行只由失败产生(答上来的路由没有 message),所以
    只给一句判据的话,读者面对的仍然是「四条错误 + 没有任何东西说明别的通了」。部分答上来时
    结论行写成 `Answered on 12 of 16 routes`;全部答上来只是带降级时不写计数(`16 of 16`
    不解释任何事),仍写 `Needs Attention`,该看的理由在详情里。
...
- **行内动作按钮(修订 2026-08-12;可见控制台承载,理由同一键安装:更新/登录有
  ...
  **当前登录账号**(修订同日,用户:「能否读取登录的账号,显式在 UI 上」):
  ...
  复制最近一条链接,`v` 把剪贴板内容(OAuth code)注入命令输入;c/v 判定=
  **孤立按键**(粘贴突发与终端转义应答都是多字节读,天然透传;「输入行为空」
  判定已删——被 Ink TUI 的终端应答污染,真机首用即失效,修订 2026-08-12 晚)。剪贴板读写由 Studio 侧
...
| `plaintext` | 真实密钥（Eye 取回的，或本次刚输入的） | 遵从 Eye 的明文/掩码开关；**聚焦即显真值**、正在输入时可见 | 正常就地编辑 |
...
**聚焦不是 reveal 动作**——A10 只授权 Eye / Copy 向服务器换取真值，所以 `stored` 态聚焦
不触发任何请求、也不显示任何真值。`plaintext` 态是另一回事：那把密钥客户端**已经握在手里**
（Eye 取回过，或就是用户刚敲进去的），聚焦显示它没有向任何人多披露一个字符，反而是能就地
改错字的前提——2026-08-20 真机点验实证：把这一档也按掩码处理，用户下一次击键就会把掩码点
并进密钥，保存被后端边界挡回 422，界面上只表现为"改不动"。
