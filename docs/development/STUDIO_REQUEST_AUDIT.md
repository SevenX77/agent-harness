...
| Run history | `GET /api/skills/{skill_id}/runs` | Skill-open cold load (Timeline and Workspace share the key) / explicit refresh / run mutation projection | OK: one cold load per skill, shared and deduped by both consumers; delete/start/resume update the shared snapshot without write-after-read refresh. Workspace joined the key on 2026-08-21 because which run of this skill is under way is a fact only the server holds (ledger C1 ④) — it reads that answer once per skill and never re-asks, so the request COUNT is unchanged and only its timing moves from Timeline-open to skill-open. | `studio-swr-policy.usage.test.ts`, `GraphCanvas.test.tsx`, `useRunHistory.revalidation.test.tsx`, `Workspace.test.tsx` |
...
FRONTEND GET /api/skills/{skill_id}/runs | ok | shared | Run-history list is an SWR cold-load key under the Studio truth policy, shared by Timeline and Workspace: one load per skill, deduped, never re-asked on mount/focus/reconnect. Workspace reads it for one purpose — adopting the run the server says is under way (ledger C1 ④) — and writes still project returned metadata without a follow-up GET.
...
FRONTEND POST /api/skills/{skill_id}/runs | ok | specific | Explicit Run command; backend returns RunMetadata and Workspace projects it into the shared run-history cache without refetching the list.
