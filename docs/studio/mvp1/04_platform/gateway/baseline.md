...
| Role resolver | Gateway resolver filters executable routes and returns resolved role/fallback chains. | `packages/graph-agent-gateway/src/graph_agent_gateway/resolve/resolver.py:resolver（L26）`, `packages/graph-agent-gateway/src/graph_agent_gateway/resolve/resolver.py:resolve_role（L33）` |
| Model resolver | graph-agent gateway adapter resolves role into chat model runtime. | `packages/graph-agent-gateway/src/graph_agent_gateway/call/resolver.py:resolve（L73）`, `packages/graph-agent-gateway/src/graph_agent_gateway/call/resolver.py:resolve（L92）` |
| Chat fallback | GatewayChatModel iterates fallback candidates, probes/classifies, and emits fallback behavior. | `packages/graph-agent-gateway/src/graph_agent_gateway/call/chat_model.py:_generate（L96）`, `packages/graph-agent-gateway/src/graph_agent_gateway/call/chat_model.py:_generate（L111）` |
