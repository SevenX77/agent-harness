...
| Settings app settings | `GET/PUT /api/settings` | App settings initialization / user save | Pending audit: must not retry-loop on missing auth or dialog open/close. | Required |
| LLM registry | `GET /api/llm/registry` | Settings/API Keys/roles consumers and domain events | Pending audit: must be cold-load plus precise registry_changed invalidation only. | Required |
| LLM roles | `GET/PUT /api/llm/roles` | Settings/Copilot role consumers and role writes/events | Pending audit: must be cold-load plus precise roles_changed invalidation only. | Required |
...
| Studio event stream | `WS /ws/events` | App-level domain event stream | Pending audit: connect/reconnect must not perform generic refetch; only event handlers may invalidate exact keys. | Required |
