...
def test_copilot_sdk_test_endpoint_persists_claude_sdk_tools_capability(
    ...
    # Seed a custom provider endpoint and route
    ...
    # Save a role specifically for copilot
    save_roles_file(
        roles_path,
        RolesData(
            roles={
    ...
    # Mock the SDK tool call probe method
    async def mock_probe_sdk_tool_call(
        endpoint,
        route,
    ) -> Any:
        probe_calls.append(route.provider_model_id)
        # Return a simulated successful tool call result
