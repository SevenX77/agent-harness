...
| **U5** | **checkpoint 嵌套拓扑 + 存储纪律** | `CK1-6`(records/state-checkpoint,2026-06-03) | base 切面→`03-checkpoint`◆ · 内层 messages 切面→`08-messages-state`◆ · 迭代切面→`02-iterate` · agent 切面→`01-agent-loop`(经 ns 挂) · 数据切面→`data-contracts`(data delta reducer 待补) | `graph_assembler.py:151`(compile checkpointer) · `state.py:214`(messages DeltaChannel) · `runner.py:resume_skill` · `cognitive_flow.py:292`(interrupt 依赖 checkpoint) | **locked**(2026-06-05) | records 深度已回填 `03-checkpoint`+`08-messages-state`;live 已有外层 super-step checkpoint + WorkflowState.messages DeltaChannel + AGENT namespace checkpoint + Engine resume_skill;data delta/compaction/Studio resume 产品仍 backlog |
...
| **U8** | **退出闸(phase 不静默成功)** | `EXIT` 决策 | 退出闸切面→`05-exit-control`◆(after_agent + NudgeInjector) · 认知切面→`03-cognitive`(finish_task / goto=END 绕闸) | `exit_control.py` + `cognitive_flow.py` + `graph_assembler.py` | **locked**(2026-06-05) | WS-E8 已接 `ExitControlMiddleware.after_agent` gate,并移除 accepted finish_task 绕闸路径;iteration leak fix 已入 |
...
| **U10** | **API 操作面(engine↔studio)** | `ARCH1`(C 层) | API 契约切面→`03-api-contract`◆ · 入口实现→`07-runtime` · 事件供给→`02-observability` · 形状供给→`data-contracts`(RunResult) | `runner.py:run_skill/predict_skill/resume_skill/evaluate_golden_baseline` · `compiler.py:compile_skill:41` | drafted(内容成段,可锁候选) | Engine 进程内 API 面 live;锁待 Studio HTTP 路由协同 + B baseline 补深 + GET /errors / resume route / iterate API 消费 |
...
- **A 达标(✅,目标写全 + demarcated,只差 impl)**:**全部 12 模块**(含 `03-api-contract`:17 块迁移源 consolidate)。本轮纠正:`skill-syntax`(io 切片 §2.10 补齐 → 语法部件全完)、`07-runtime`(§2/§3 顶层入口 + bootstrap + public API 契约已成文)、`01-agent-loop`/`03-assemble`/`03-cognitive`(§2 机制 + §3 契约 + §4 决策依据齐;create_agent 迁移 cluster 无 PM 原话是**内部工程迁移**、§4 已诚实披露,非缺口)。
- **A 不达标**:**无**——12 模块 alignment 全 A 达标(api-contract 17 块已 consolidate)。
- **B 仍 ◐**:**无**——`03-api-contract` baseline 已填(2026-06-06),12 模块 B 全 ✅。
...
> **内容层面:engine 侧 12 模块 alignment + baseline 全齐**(2026-06-06 对账完成:§3.1 矩阵 A/W glyph 已同步 ✅、api-contract baseline 已填、`_migration-src` 已标 archived)。剩 = **U10 单元锁**(studio HTTP 路由协同)+ **impl**(各模块 §8 refactor-target 归 kiro)。锁 / audited-ready 留实施后(见 Gemini 复核 + `task-spec-standard §四`)。
...
| 模块 | W | B | A | 缺口 / 证据 |
|---|---|---|---|---|
| `01-contract/01-physical-layout` | ✅ `mvp1-alignment.md:115,123-126` | ✅ `baseline.md:6,12`; code `loader.py:146`, `runner.py`, `emit.py:15` | ✅ `mvp1-alignment.md:115,123-138`(.workspace 户型成段;golden eval SDK 已落地;test_inputs SDK 仍 backlog) | `.workspace` 户型已写;`evaluate_golden_baseline`/`golden` Engine eval live;`test_inputs` Engine SDK 未落地(backlog)。 |
| `01-contract/02-skill-syntax` | ✅ `mvp1-alignment.md:574,581-588` | ✅ `baseline.md:6,12`; code `loader.py:146/1499/1592`, `manifest.py:108/121/143/152/162`, `mentions.py:21` | ✅ `mvp1-alignment.md`(resource §2.8 / iterate §2.9 / io 切片 §2.10 全补,语法部件齐) | 语法部件全齐(§2.1-§2.10);执行/校验 impl 归 kiro。 |
...
| `02-mechanism/03-assemble` | ✅ `mvp1-alignment.md:22-26`(§4 工程决策依据;create_agent cluster 无 PM 原话已披露); design `agent-loop-planA-create-agent-migration.md:26,36,50` | ✅ `baseline.md:4,20-38`; code `graph_assembler.py:88/158/423` | ✅ `mvp1-alignment.md`(§2 装配机制 + create_agent 构造收口 + cross-ref agent-loop;skill-syntax 真空已清) | 内容齐;_build_skill_node 收口 create_agent 实现归 kiro。 |
| `02-mechanism/04-run-outer/01-graph-exec` | ✅ `mvp1-alignment.md:32,40-42`; overview `00-architecture-overview.md:113` | ✅ `baseline.md`(+11-io 现状:`loader.py:528` 子图 io 1:1、`io/manager.py:108` 落盘、`io/storage.py:149`);code `graph_assembler.py:325/363`, `state_mapper.py:37`, `actions.py:18/49` | ✅ `mvp1-alignment.md`(LOGIC LE1-3 + 11-io E1-E4 + AGENT 委派/run_context 成段;nudge 归 exit-control;U4 锁) | LOGIC 决策已定;11-io 已吸收、nudge 归 exit-control;LE 重构 + 11-io impl 归 kiro。 |
...
| `02-mechanism/04-run-outer/03-checkpoint` | ✅ `mvp1-alignment.md`(CK1-6 + 6 段 PM 原话回填) | ✅ `baseline.md`(B 成段+WS-E5/E7 回写;code `checkpointer.py:39`, `runner.py:resume_skill`, `graph_assembler.py`, `state.py:212/214`, `phase_executor.py:82`) | ✅ `mvp1-alignment.md`(records 深度回填:递归拓扑/delta-compact/durability/D-test;codex 纠"目标当现状"+加现状框;U5 锁) | Engine checkpoint namespace + `resume_skill` live;data delta/compaction 和 Studio HTTP/UI resume 仍 backlog。 |
| `02-mechanism/05-run-inner/01-agent-loop` | ✅ `mvp1-alignment.md:21-25`(§4 工程决策 AL1/AL2 + 审计披露无 PM 原话因属内部迁移); design `agent-loop-planA-create-agent-migration.md:36,80` | ✅ `baseline.md:4,20-44`; code `graph_assembler.py:423` | ✅ `mvp1-alignment.md`(§2 create_agent 迁移机制全 + §3 契约;live 手写 loop=目标态 demarcate) | 内容齐;手写 loop→create_agent 实现归 kiro。 |
...
| `02-mechanism/05-run-inner/03-cognitive` | ✅ `mvp1-alignment.md:24-29`(§4 CG1-3 工程决策 + 审计披露); design `agent-loop-planA-create-agent-migration.md:74,80,101` | ✅ `baseline.md:12,22-31`; code `cognitive_flow.py:55`, `finish_task.py:21/30`, `md2json.py:26`, `md_to_json.py:515` | ✅ `mvp1-alignment.md`(§2 finish_task/三态分流/输出/prompt 全;rich 三态未接 live=目标态) | 内容齐;rich 三态接 live + md2json 消重归 kiro。 |
| `02-mechanism/05-run-inner/04-tools` | ✅ `mvp1-alignment.md:22-31`(§4 工程决策依据;create_agent cluster 无 PM 原话已披露); graph-exec LE1-3 `04-run-outer/01-graph-exec/mvp1-alignment.md:38-42` | ✅ `baseline.md:4,20-28`; code `actions.py:49/60`, `tool_error.py:12` | ✅ `mvp1-alignment.md`(§2 tool 生命周期+ToolError 行为成段,demarcated;U3 锁) | action/tool 不统一已定;ToolError 实现(tool_error.py no-op)归 kiro。 |
| `02-mechanism/05-run-inner/05-exit-control` | ✅ `mvp1-alignment.md:21,28-30` | ✅ `baseline.md`; code `exit_control.py`, `cognitive_flow.py`, `graph_assembler.py` | ✅ `mvp1-alignment.md:15-18,39`(目标成段,U8 锁) | ExitControl after_agent gate 已由 WS-E8 live;Studio 无额外 ownership。 |
| `02-mechanism/05-run-inner/06-golden-eval` | ✅ `mvp1-alignment.md:24,30-32`; overview `00-architecture-overview.md:93` | ✅ `baseline.md`(WS-E7 回写);code `runner.py:evaluate_golden_baseline`, `_predict_internal/golden_eval.py`, `runner.py:resolve_generation`, `strategy.py`, `interception.py:29`(skeleton) | ✅ `mvp1-alignment.md`(反转重写 + WS-E7 Engine eval live) | Engine `evaluate_golden_baseline` live;Studio HTTP/UI、空 template、predict 拦截搬 engine、旧 hash warn 退役仍 backlog。 |
| `02-mechanism/05-run-inner/07-subagent` | ✅ `mvp1-alignment.md:21,27-29`; uncovered `uncovered-areas.md:50-70` | ✅ `baseline.md:4,20-29`; code `graph_assembler.py:1057/1120/1158`, `subagents.py` | ✅ `mvp1-alignment.md`(§1-§8 全:派发机制/PM 原话/SA1-3/测试;lifecycle 事件 A2、断层#7=impl·协同归 kiro) | 内容齐;live 仍内联(未收 wrap_tool_call)+ lifecycle 事件 = impl。 |
| `02-mechanism/05-run-inner/08-messages-state` | ✅ `mvp1-alignment.md`(HS1-4 + PM 原话回填) | ✅ `baseline.md`(WS-E5/E7 回写;code `state.py:214/28`, `runner.py:resume_skill`, `llm_phase_node.py:275/381/809`, `cognitive_flow.py:292`) | ✅ `mvp1-alignment.md`(records 深度回填:delta/compact 正交/snapshot_frequency=50/CK6;codex 纠"目标当现状"+现状框、AgentState→WorkflowState;U5 锁) | DeltaChannel + Engine HITL resume live;summarization/compaction 和 Studio resume product 仍 backlog。 |
| `02-mechanism/06-seam/01-models` | ✅ `mvp1-alignment.md:21,27-29`; design `agent-loop-planA-create-agent-migration.md:152,165` | ✅ `baseline.md:4,20-24`; code `graph_assembler.py:581`, `interception.py:29/61` | ✅ `mvp1-alignment.md`(predict_context 透传/usage 归零/structured-output mock 约束已迁,源 uncovered #1;create_agent 标目标态,codex 复审) | 13-models 主体 + predict 约束已迁;predict mock 搬回引擎(D2)=impl 归 kiro。 |
...
| `02-mechanism/07-runtime` | ✅ `mvp1-alignment.md:23,29-30` | ✅ `baseline.md:10,20-29`; code `runner.py:run_skill/predict_skill/resume_skill/evaluate_golden_baseline`, `__init__.py` public surface, `test_public_api_contract.py` | ✅ `mvp1-alignment.md`(§2/§3 顶层入口 + bootstrap + public API 契约成文;run/predict/resume/golden eval=模式) | 内容齐;public API live+干净,bootstrap 文档化 + 死簇清理归 backlog。 |
| `03-api-contract` | ✅ `mvp1-alignment.md:44,50-51` | ✅ `baseline.md`(WS-E7 回写:Engine resume/golden API live;Studio 12/13 live·resume route 501) | ✅ `mvp1-alignment.md`(执行签名/返回/事件/HTTP 端点总索引/Golden/Iterate-Resume/Compile API 面全成段) | Engine API 面 live;Studio HTTP resume、GET /errors、iterate 图级 API 消费仍 backlog。 |

### 3.2 未迁入 `_migration-src` 源清单(archive 前必须补)

| 迁移源 | 未迁状态 | 应归目标 | 证据 |
|---|---:|---|---|
| `_migration-src/api-engine-studio-contract.md` | **19/19 已迁**(17 块 consolidate 入 03-api-contract §2.1/§3/§3.1-3.3,2026-06-06;studio HTTP 路由引用不 own) | `03-api-contract/mvp1-alignment.md` | `api-engine-studio-contract.md` §1-§7 全 |
...
- ~~`01-contract/02-skill-syntax`:仅 io 切片语法仍真空~~ → ✅ io 切片 §2.10 已补(2026-06-06);语法部件全齐(resource §2.8 / iterate §2.9 / io §2.10),**无剩余真空**。
...
- ~~`02-mechanism/07-runtime`:顶层入口契约/bootstrap 未成段~~ → ✅ §2/§3 已成段(2026-06-06);bootstrap 文档化 + 死簇 `GraphAgentHarness` 清理 = impl 归 kiro。
