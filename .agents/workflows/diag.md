---
description: 卓越工程代码美学与架构诊断规约 (Anti-Laziness Diagnostics Oracle)。本规约约束主控 Agent 执行项目根目录顶级 `code-diagnostics/` 独立体检套件：所有产物写入 `code-diagnostics/output/{run_id}/`，并通过主控调度子代理完成高密度代码美学体检。
---

## 🛠️ 第一阶段：动态报告创建与硬性扫描 (Setup & Static Audit)

主控接收到体检打分任务时，必须生成一个唯一的精确 UTC `{run_id}`（如 `20260601_082943`），并按顺序执行以下注入式动态操作：

1. **运行结构生成器**：
   ```bash
   python3 code-diagnostics/build_tree.py --file code-diagnostics/output/{run_id}/diag_report_{run_id}.md
   ```
   它会在 `output/{run_id}/` 目录下生成该次审计的基准清单文件，并将全仓待审文件初始化为待审计状态 `[ ]`。
...
### 3. 子代理落盘与回填
主控按模块分批派发 subagent，每批必须把结果写入 `code-diagnostics/output/{run_id}/findings/batch_{k}.json`。所有批次完成后运行模型无关回填器：
```bash
...
```
它会合并 findings JSON、回填健康分与微观证据，并完成 final score 折算。
...
1. **天然归档**：体检文件即为 `output/{run_id}/diag_report_{run_id}.md`，无 latest 冗余复制。
2. **不入库**：`output/` 已被 `.gitignore` 忽略，报告与中间产物不污染仓库历史。
