...
# Register schemas locally or import from sibling
...
def validate(output: dict, state_slice: dict, **kwargs) -> dict:
    ...
    tension_results = [
        r.model_dump() for r in md_to_json(raw1, TensionEmotionVibeResult)
    ]
    ...
    if c_events:
        ...
        system_results = [
            r.model_dump() for r in md_to_json(raw2, SystemEvolutionResult)
        ]
    ...
    prop_results = [r.model_dump() for r in md_to_json(raw3, PropChangesResult)]
    ...
    arc_results = [r.model_dump() for r in md_to_json(raw4, EmotionalArcsResult)]
    ...
    foreshadowing_results = [
        r.model_dump() for r in md_to_json(raw5, ForeshadowingResult)
    ]
    ...
    spatiotemporal_results = [
        r.model_dump() for r in md_to_json(raw6, SpatiotemporalResult)
    ]
    logger.info(
