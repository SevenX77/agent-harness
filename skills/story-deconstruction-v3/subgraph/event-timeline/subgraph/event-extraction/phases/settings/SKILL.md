...
io:
  inputs:
    type: object
    required: [formatted_paragraphs, events_raw, chapter_number, chapter_segmentation, event_timeline]
    properties:
...
<goal>
请从以下事件段落中识别世界设定内容，提炼为知识库，并校验事件结果。校验器会将最终结果写成 chapter_event_timeline。
