...
1. **mvp0 只允许说"契约已迁"**:
   ...
   - **不可说"mvp0/engine 迁移整域可 deprecated"**:`_migration-src/`→`02-mechanism` 尚未清完,尤其 `api-engine-studio-contract`、`records/state-checkpoint-storage-model`、`records/uncovered-areas` 仍有未迁块;`09-golden-eval`(baseline 已迁、alignment 待退役确认)、`11-io-and-edge-ops`(已分发入 graph-exec+observability,codex 复审)基本清完(见 §3.2)。
2. **锁迁移序(真实版)**:
