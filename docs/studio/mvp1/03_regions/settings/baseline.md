...
doc: baseline
status: FROZEN（WS-4 更新后现状；Settings shell live；前后端/界面已使用六态 provider/model state，Role Test 完成后刷新后端投影；Copilot tab 还有保存/role-key drift ⚠️。）
...
> **现状一句话**: Settings shell live；前后端/界面已使用六态 provider/model state；LLM Roles 的 Role Settings 和 Role Test 已回到后端投影作为最终事实，Copilot tab 还有保存/role-key drift ⚠️。
...
| Refresh | Settings refreshes credentials/roles on focus, registry websocket events, and completed persisted Role Test jobs. | `apps/studio/frontend/src/components/studio/settings/SettingsPage.tsx:refreshLoadedLlmRolesProjection`, `apps/studio/frontend/src/components/studio/settings/LlmRolesTab.tsx:runPersistedRoleTestJob` |
...
| Provider/model state | Frontend/backend state enum uses `ready/historical_ready/untested/failed/cooling_down/off`; `needs_setup` is no longer a state. | `apps/studio/frontend/src/api/llm.ts:ProviderUiState`, `apps/studio/backend/app/services/llm_state_projection.py:ProviderUiState` |
| Role Settings | Role card inline settings save model fallback, Thinking `off/preferred/required`, and context/output token intent without exposing provider-order or cost-priority controls. | `apps/studio/frontend/src/components/studio/settings/llm-roles/RoleSettingsDialog.tsx:RoleSettingsPanel` |
...
|---|---|---|
| 六态 | UI/API 已使用六态并移除 `needs_setup` 状态。 | 六态标签和错误 copy 一致 |
| role materialization UI | Role Test 完成后会刷新后端 roles/model groups 投影；短暂进度仍在本地内存中。 | 显示来自 ③b/③a 单一投影，不本地猜真相 |
| role settings intent | Role Settings 已覆盖 Thinking 三态和 context/output token intent。 | UI 保存用户意图，materializer/gateway 生成最终 runtime settings |
| Copilot settings | Copilot test/save 仍有路径差异 ⚠️ | 真实 SDK smoke 结果在 Copilot tab 可见 |
| Properties role shortcut | Settings/LLM API 已提供同一投影与 persisted Role Test 契约；Properties UI 文件仍受 WS-2 文件锁约束，未在本次接入。 | Properties role 行复用同一状态与测试契约 |
> **验"是否按目标改了"**：1. 六态；2. role materialization UI；3. role settings intent；4. Copilot settings；5. Properties shortcut。
