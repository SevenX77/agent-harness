...
- 机制: subgraph references resolve by **绝对 path**(engine skill-syntax §2.1); **membership 先判路径包含——子图绝对 path 已在父 skill 工作目录树内(默认 `subgraph/<name>/` 即是)就视为已在工作区、不重复加;仅当子图 path 在工作区外**, Assets lets the user add that folder into the workspace.
...
| SKILL_WORKSPACE-3 | 子图 membership | 单元 `subgraph-path-inline-drilldown`（消费/引；owner 非 skill-workspace）；**为什么**：子图按 path 解析、随便放哪，copilot cwd 须纳入子图 path(D7)——但**先判路径包含:子图绝对 path 已在父 skill 工作目录树内就不重复加,只有工作区外的子图才追加 cwd** |
