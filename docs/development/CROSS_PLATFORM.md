...
- **读别人写的文件用 `encoding="utf-8-sig"`，读我们自己写的文件用 `"utf-8"`。**
  ...
  编辑器往我们手里塞什么：有些工具会在文件开头写一个 UTF-8 字节序标记（BOM，
  字节 `EF BB BF`）。那三个字节属于**编码**，不是正文的
  第一个字符；用 `"utf-8"` 读进来，它会变成一个 `\ufeff` 字符卡在最前面。

  **签名不是当今 Windows 的默认行为，但“不是默认”不等于“不会发生”**
  （2026-08-31 核对，修正本节此前“记事本和 PowerShell 重定向默认写 BOM”的
  说法）：现在的记事本默认存无签名 UTF-8，Windows PowerShell 5.1 的 `>`
  重定向默认写的是 UTF-16LE——后者 `utf-8-sig` 同样读不了，我们也不打算支持。
  仍然会产出签名的是这些：记事本“UTF-8 with BOM”这个显式存档选项、Windows
  PowerShell 5.1 的 `Out-File` / `Set-Content -Encoding utf8`（PowerShell 6+
  已改为不带签名）、被配置成加签名的编辑器、表格软件的“CSV UTF-8”导出。
  **一个用户用了其中之一就够了**，而他拿到的报错既说不出原因也说不出解法。
  所以这条规则的依据从来不是“默认如此”，是下面这次实测：
