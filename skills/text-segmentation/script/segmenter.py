...
def prepare_chapter(context: dict) -> str:
    ...
    return f"Chapter {chapter_number} prepared with {len(lines)} lines"
...
def parse_segmentation_output(raw_output: str, context: dict) -> str:
    ...
    for _i, line in enumerate(lines):
        ...
        para_match = re.match(r"^\- \*\*段落(\d+)（([ABC])类.*?）\*\*：(.+)$", line_stripped)
        ...
        if para_match:
            ...
            inline_range = re.search(
                r"(.+?)\s*行号[：:]\s*(\d+)\s*[-–—~至到]\s*(\d+)\s*$", desc_part
            )
        ...
        if current_paragraph and current_paragraph["start_line"] is None:
            range_match = re.match(r"^\s*行号[：:]\s*(\d+)\s*[-–—~至到]\s*(\d+)", line_stripped)
            if not range_match:
...
def log_ambiguous_segments(
    segment_index: int, reason: str, confidence: float, context: dict
) -> str:
...
def validate_segmentation(context: dict) -> tuple[bool, list[str]]:
    ...
    for seg in segments:
        ...
        if seg_type == "A":
            ...
            if 0 < verb_count < 3:
                msg = (
                    f"Segment {seg_idx}: A-class contains {verb_count} "
                    "narrative verbs (boundary case)"
                )
                ambiguities.append(msg)
                report = {
                    "segment_index": seg_idx,
