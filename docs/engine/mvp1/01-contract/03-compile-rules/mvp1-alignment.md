...
> **Tier**: 契约层 A(声明式,喂 copilot) | **Owns**: 编译/装配/运行生命周期契约 + 全部校验规则(DAG/IO/mention/purity/golden/iterate)+ `[F-v3-*]` 错误码全表 | **现状**: mvp1 自承载,代码 registry 89 码 | **Related**: `skill-syntax`(被校验语法)· `01-compile`(扫描器实现)· `invalidation`/`06-golden-eval`(golden eval 期)· `02-iterate`(iterate 新码目标)· `03-api-contract`(CompileResult)
...
本文件现在是 compile-rules 的 mvp1 SSOT，不再链接 mvp0 spec 当权威。89 个现有码以 `packages/graph-agent/src/graph_agent/core/error_registry.py:ERROR_REGISTRY` 为代码 baseline；本文件保留迁移源里的「具体原因 / 修复建议」，避免旧文删除后丢失解释语义。
...
| `[F-v3-agent-llm-role-missing]` | 编译期 | Agent phase 解析不到任何 llm role(节点与整图默认皆未设置;引擎无兜底角色,2026-08-31 用户裁决) | 为该 phase 设置 llm_role,或在 GRAPH.md 设整图默认(子图需各自声明) | [Agent](../02-skill-syntax/mvp1-alignment.md#2-语法部件清单--mvp1-写入状态) |
