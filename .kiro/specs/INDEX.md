...
| [`studio-ah-state-contract-v1/`](./studio-ah-state-contract-v1/) | Draft (requirements+design+tasks 修订完成,a4 已终审 ACCEPT,待 PM 终审) | **Studio ah 状态检测/清理切到 ah v1.4.0+ 结构化状态合约**:events 为主决策面+status 为 fallback(daemon-absent 结构化差异) · `runtime_state` 覆盖 starting/degraded 相位(degraded 必须有可用 Open 路径) · config 所有权二分类(workspace-owned 只读/Studio-managed 全生命周期,避免误杀本仓 operator 自己的编队) · `AH_STATE_DIR` 类环境变量钳制+快照身份校验 · per-assistant 状态 payload。operator 评审见 `operator-review-findings.md`(F1-F9),逐条溯源见 `REVISION-TRACE.md` | [`docs/studio/mvp1/03_regions/copilot/ah-orchestration-design.md`](../../docs/studio/mvp1/03_regions/copilot/ah-orchestration-design.md) |
