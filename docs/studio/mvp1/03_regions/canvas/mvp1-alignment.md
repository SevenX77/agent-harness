...
| ID | 决策 | 动机 |
|---|---|---|
| CANVAS-1 | 节点态 | 对齐 `subgraph-path-inline-drilldown` 设计单元，保证 region 切面能被测试回扣 |
| CANVAS-2 | dot 黑板 | 对齐 `subgraph-path-inline-drilldown` 设计单元，保证 region 切面能被测试回扣 |
| CANVAS-3 | 子图 inline | 对齐 `subgraph-path-inline-drilldown` 设计单元，保证 region 切面能被测试回扣 |
