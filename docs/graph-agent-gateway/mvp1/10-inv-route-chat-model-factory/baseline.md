...
4. `ModelResolver.resolve` 当前正常分支直接返回 `GatewayChatModel`；`GatewayChatModel` 是 LangChain `BaseChatModel` 外壳，用来保留 fallback、probe、熔断、usage 和 metadata 编排行为(`packages/graph-agent-gateway/src/graph_agent_gateway/resolver.py:135-146`; `.kiro/specs/studio-llm-gateway-redesign/client-layer-decision-record.md:28-38`)。
...
9. `LLMClientManager._call_with_token_escalation` 在 provider 返回截断 finish reason 时提高 token budget 重试；它是现状真实调用路径的一部分，MVP1 不能随 `_call_*` 一起丢掉(`packages/graph-agent-gateway/src/graph_agent_gateway/client_manager.py:990-1012`; `.kiro/specs/studio-llm-gateway-redesign/client-layer-decision-record.md:200-204`)。
...
alignment 目标不是删除 `GatewayChatModel`。权威决策已否决“resolver 直接裸返回 ChatX”的方案，要求保留 `GatewayChatModel` 做编排外壳，只把每条 route 的真实调用换成原生 LangChain ChatX(`.kiro/specs/studio-llm-gateway-redesign/client-layer-decision-record.md:28-38`)。

## 决策原因

1. 需要拆开“编排”和“调用”。权威记录把编排输入定义为 role/model override，输出定义为 `ResolvedRoute` 链；调用层输入是一条 `ResolvedRoute` 加 messages，输出是 `AIMessage`/结果(`.kiro/specs/studio-llm-gateway-redesign/client-layer-decision-record.md:45-60`)。
2. 现状把调用揉在 fallback loop 里。`GatewayChatModel._generate` 在同一个函数里做 probe、熔断、调用、usage、metadata、异常分类，导致 ChatX 替换很难只改调用层(`packages/graph-agent-gateway/src/graph_agent_gateway/gateway_chat_model.py:111-271`; `.kiro/specs/studio-llm-gateway-redesign/client-layer-decision-record.md:101-117`)。
3. 旧 client_manager 调用层问题集中在消息转换和 provider payload。权威记录点名 `_langchain_messages_to_dict` 会把带 tool calls 的空内容 `AIMessage` 转成空 content dict，这是这轮迁移要修的调用层根因(`.kiro/specs/studio-llm-gateway-redesign/client-layer-decision-record.md:34-38`; `packages/graph-agent-gateway/src/graph_agent_gateway/gateway_chat_model.py:661-692`)。
4. client_manager 不能整块删。它仍承载 probe、provider-down TTL、usage 统计等编排/观测职责；MVP1 只退役消息转换和 provider 调用部分(`.kiro/specs/studio-llm-gateway-redesign/client-layer-decision-record.md:82-97`; `packages/graph-agent-gateway/src/graph_agent_gateway/client_manager.py:53-132`; `packages/graph-agent-gateway/src/graph_agent_gateway/client_manager.py:310-368`)。

## 代码索引 clues

- `RouteChatModelFactory`:当前不存在；用途应是一条 `ResolvedRoute` 到原生 ChatX 实例的构造器，权威记录把它定义为调用层核心(`.kiro/specs/studio-llm-gateway-redesign/client-layer-decision-record.md:157-163`)。
...
2. 待办:base_url 保存时归一化属于 03 模块；10 模块只记录调用时幂等双保险，不能在工厂里替代保存侧修复(`.kiro/specs/studio-llm-gateway-redesign/client-layer-decision-record.md:169-180`)。
