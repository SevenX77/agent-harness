...
- **读取(read)只在 `"declined"` 时停**——`"unset"` 仍允许读取。理由:读取不带走这台机器上的任何东西,而"能直接用上社区已验通的参数"正是对话框承诺的好处,首屏就把它关掉只会让第一次体验更差,却换不来任何隐私收益。实现都在 `apps/studio/backend/app/services/community_catalog_runtime.py`:`sync_verified_community_catalog_into_credentials`(应用面覆盖启动期自动同步与 `POST /api/llm/catalog/sync-verified` 手动同步)与 `merge_synced_catalog_into_created_routes`(见下条),两处的门控判断一致。
- **读取触发器有三个:启动、手动同步端点、以及「Test/探测创建了新 route」的同一写事务**(2026-08-29 补,批示轮三 R3-4 / J-01.I)。原设计只有前两个,fresh 旅程实测(2026-08-27)坐实了缺口:Phase 5 不缓存未匹配的社区 evidence,首启同步时 routes 还是空的、42 条记录全部落空;用户配 key + Test 建出 route 后,社区蓝要等**下一次重启**的启动同步才出现——「当场性」断裂(用户批示③「v4flash为什么是灰的?社区同步数据呢?」)。修订后:`test_endpoint` 与 `test_endpoint_models` 在本次测试**确实创建了新 route** 时,于同一次 `save_credentials` 之前调用 `merge_synced_catalog_into_created_routes` 现拉目录并把 probe-verified evidence 合并进新 route——写响应即投影 Previously Connected 蓝,不用重启。边界:没有新建 route 的 Test 不触发远端拉取(稳态点 Test 不付网络代价);拉取/验签失败只记 runtime activity、绝不让触发它的 Test 失败;`"declined"` 门控与上一条同一判断。
