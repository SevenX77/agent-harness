...
def refresh_runtime_config(skill_dir: Path) -> dict[str, Any]:
    ...
    manifest, root_bindings, phase_bindings = _scan_import_files(workspace_dir_for_runtime(skill_dir))
...
def _scan_import_files(workspace_dir: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    root_bindings = _bindings_from_entries(root_entries)
    ...
    if phase_root.is_dir():
        for phase_dir in sorted(path for path in phase_root.iterdir() if path.is_dir()):
            ...
            phase_entries[phase_dir.name] = entries
            phase_bindings[phase_dir.name] = _bindings_from_entries(entries)
    return {"root": root_entries, "phases": phase_entries}, root_bindings, phase_bindings
...
def _bindings_from_entries(entries: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for entry in entries:
        ...
        if kind == "dir":
            ...
            if isinstance(nested, list):
                bindings.update(_bindings_from_entries(nested))
            continue
        if kind == "batch":
            ...
            if isinstance(field_name, str) and field_name:
                ...
                bindings[field_name] = {key: value for key, value in binding.items() if value is not None}
            continue
        path = entry.get("path")
        ...
        for field in fields:
            ...
            bindings[field["name"]] = {key: value for key, value in binding.items() if value is not None}
    return bindings
