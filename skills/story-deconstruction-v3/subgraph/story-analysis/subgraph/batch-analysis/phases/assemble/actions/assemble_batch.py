...
def assemble_batch(inputs) -> dict:
    """Assemble analysis results and return the next analysis_state value."""
    ...
    if not events:
        ...
        foreshadowing = {
            r.get("event_id", ""): r
            for r in inputs.get("foreshadow_results", [])
        }
        spatiotemporal = {
            r.get("event_id", ""): r
    ...
    updated_state = _normalized_accumulator_state(
        inputs.get("accumulator_state", {})
    )
    if inputs.get("entity_registry") is not None:
        updated_state["entity_registry"] = copy.deepcopy(inputs["entity_registry"])
    if inputs.get("entity_aliases") is not None:
        updated_state["entity_aliases"] = copy.deepcopy(inputs["entity_aliases"])

    char_results = inputs.get("character_results", [])
    prop_results = inputs.get("prop_results", [])
    fore_results = inputs.get("foreshadow_results", [])
    ...
    for r in char_results:
        ...
        if char_id:
            updated_state["known_characters"][char_id] = {
                "name": r.get("name", ""),
                "current_state": r.get("current_state", r.get("state", "")),
            }
            updated_state["character_latest_states"][char_id] = r.get(
    ...
    for r in prop_results:
        ...
        if prop_id:
            updated_state["known_props"][prop_id] = {
                "name": r.get("name", ""),
                "current_state": r.get("current_state", r.get("state", "")),
            }
    ...
    for r in fore_results:
        if r.get("is_resolved"):
            updated_state["open_foreshadowing"] = [
                f
                for f in updated_state["open_foreshadowing"]
                if f.get("foreshadowing_id") != r.get("foreshadowing_id")
            ]
        else:
            updated_state["open_foreshadowing"].append(r)

    for r in arc_results:
        arc_id = r.get("arc_id")
        if arc_id and r.get("is_active", True):
            active_arc_ids = [
                a.get("arc_id") for a in updated_state["active_arcs"]
            ]
            if arc_id not in active_arc_ids:
                updated_state["active_arcs"].append(r)

    for r in spatiotemporal_results:
        if r.get("location"):
            updated_state["location_registry"][r.get("location")] = r.get(
                "normalized_location", r.get("location")
            )
        if r.get("timestamp"):
            updated_state["time_tracker"][r.get("event_id", "")] = r.get(
                "timestamp"
            )

    current_batch = inputs.get("current_batch") or {}
    updated_state.setdefault("batch_history", [])
    updated_state["batch_history"].append(
        {
            "batch_index": current_batch.get("batch_index"),
            "chapter_range": current_batch.get("chapter_range"),
            "events": batch_result,
            "continuity_warnings": inputs.get("continuity_warnings", []),
        }
    )
