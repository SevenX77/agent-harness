...
- **机制 / 数据流**(原始混合凭证 → 标准 endpoint list + canonical endpoint_id；③b 公共，整块缺漏补设计)：
  ...
  5. 生成 canonical endpoint_id(③b):为每条拆出的 endpoint 生成确定的 canonical id。**规则 = `{slug}-{protocol}[-n]`**(`slug` 来自 provider/host 收敛，`protocol` 区分同 host 多协议，`-n` 处理重名)。`standardize_endpoint_candidates` 已按 canonical base_url 稳定排序后分配 suffix,并可避开既有 endpoint id;v3→v4 migration 的历史 host id 规则已保留为 gateway `legacy_v3_endpoint_id`,避免新 canonical 规则破坏旧迁移输出。
