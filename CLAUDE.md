...
> **Studio feature task (UI or frontend-driven full-stack)?** Before planning
> or touching it, first read
> [`apps/studio/frontend/CLAUDE.md`](apps/studio/frontend/CLAUDE.md) — the
> single-agent feature SOP that swaps the heavy multi-agent PM workflow for a
> lightweight loop covering frontend + studio backend (+ engine/gateway when
> the design demands). Claude Code only auto-loads that nested file *lazily*
