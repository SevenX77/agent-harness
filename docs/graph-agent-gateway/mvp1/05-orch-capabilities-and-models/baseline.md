...
| `packages/graph-agent-gateway/src/graph_agent_gateway/registry/profile_selector.py` | `select_verified_profile`：用于在一条 route 的 ready verified profiles 中选出最符合 runtime intent 的调用 profile。`ProfileSelectionError`：用于表达没有任何 verified profile 能满足请求意图。 | 100%,含 reasoning/image modality 选择和排序。 |
| `packages/graph-agent-gateway/src/graph_agent_gateway/registry/lint.py` | `lint_role_routes`：用于检查显式 role fallback chain 上每条 route 是否满足 lint requirement 和 runtime setting 能力边界。`capability_key_for_lint`：用于把 lint 名称映射到 normalized capability key。 | 100%,含 runtime setting lint。 |
