...
- Role provider 的全局可用性和 API key 状态必须通过后端 DTO 暴露的 owning `endpoint_id`（或兼容旧数据时的 route id 前缀 fallback）映射到 credential endpoint；不得把 `route_id` 本身当作 API Keys credential id。Role card 顶部 `Test` 是 persisted Role Test：必须先 flush 当前 role autosave（包括 debounce timer 里尚未发出的 snapshot），再调用 `/api/llm/roles/{role_name}/test`。Role Test surface 只展示 provider label、三态结果、role capability/降级/冷却等用户可行动诊断，不展示 raw route/endpoint/canonical id；后端需要实际 probe 多个 provider route 时应并发执行，而不是按 runtime fallback 串行短路。
...
- Role card 内的 Model Group row 只表达 fallback 顺序和 provider route 组成，不渲染 `Connected`/`Failed`/`Unavailable` 这类 aggregate 状态 badge，也不承载 settings gear；thinking、output-token 等偏好设置必须放在 Role header 展开的内联配置 panel。
- Role 级配置不使用 modal；点击 role card header 的 role title 区域展开/收起内联配置 form。Provider order 不再作为配置项暴露，实际顺序由 provider row 拖拽决定并保存为 manual order。Thinking 用开关表达 Required/Off。Output token 只保留一个可空数字输入，必须在输入附近展示当前选中 provider routes 已知的 output cap min/max；如果部分或全部 route 尚未探测，明确提示先 Test。`Use route max and mark Limited` 表示继续运行但按 provider cap 输出并在 role test 中显示 Limited；`Block that route` 表示该 route 不进入可运行路径。
- Role 删除必须使用本地 `DeleteConfirmDialog` 二次确认，并调用 persisted `DELETE /api/llm/roles/{role_name}`。不要只从本地 draft 删除后依赖 `PUT /api/llm/roles`，因为该 PUT 是合并语义，缺失 role 会被后端保留。
...
- API Keys 的 official provider 卡片只暴露一个名为 `Test` 的动作，位置在 API key 行右侧；它调用后端 official endpoint test，由后端先读取 provider catalog，再按官方信息和 fallback 候选逐个 probe method/profile/capability。测试成功的语言/推理 route profile 才进入 `Available Routes`、registry routes 和 LLM Roles；catalog-only 或生成式多模态候选进入后端 capability library，不得显示在 LLM Roles 的 Available Models。official provider 卡片不得再渲染手动 `Endpoint test` 输入行。
- API Keys 的 official provider `Test` 必须走后端 job 化测试接口：启动请求只返回 job id/compact progress，前端轮询 compact job status，不得在每个轮询或最终测试响应中返回完整 registry。大量模型 provider 的 probe 必须由后端限并发、分批落盘；后端 compact `available_models` 必须包含本次 catalog 的全部候选及其当前 probe status（例如 `unverified_manual`、`verified`、`failed`），前端每次 poll 都要合并到当前 provider test result，让用户看到状态逐步写入。
- API Keys 的 official provider 测试结果列表必须显示为 `Available Routes`，不是 `Available Models`。每个标签文本仍使用真实 `provider_model_id`，但标签状态来自后端 `ProviderRoute.status` / verified profile / capability library 候选结果：已验证 route 使用 `Tag variant="success"` 绿色 outline；只有明确执行 probe 且失败的 route 使用 destructive/red 边框；catalog-only、非语言 route、生成/embedding/audio/video 等尚未按对应 official protocol 验证的候选应使用 `unverified_manual` 中性标签。前端不得只凭 catalog list 或 model id 推断 route 已可用，也不得在最终结果里丢掉 catalog-only 候选；这些候选可留在 API Keys 诊断列表，但不得进入 LLM Roles 的 Available Models。
- API Keys 的 official provider `Available Routes` 标签必须表达 probe 进度和模型类型：测试过程中尚未拿到最终 probe message 的候选 route 使用与 LLM Roles provider row 一致的边框流动动画；后端标记为生成式多模态、audio/realtime、embedding、video、translation、3D 等 capability-library 类型的候选使用 `Tag variant="info"` 蓝色 outline，并用本地 Radix `Tooltip` 展示模型类型。verified route 的 tooltip 必须展示已验证 profile capability 类型（例如 text chat、reasoning、image input）和 method，而不是 profile 数量。失败候选继续使用 destructive/red 边框。
- API Keys 的 official provider `Test` 成功 toast 必须按已验证 route 数量汇总，不得把返回的 route/model 总数写成 verified；如果返回列表中存在 failed、unverified 或未知状态，应在 toast 中明确显示 not verified 数量。
- API Keys 的 third-party/custom provider 仍保持模型列表探测和端点连通探测拆开：`Get Models` 只调用 provider 的 models-list API，按钮不得使用 primary variant，成功后只刷新当前参数匹配的 `Available Models` / `Available SDKs`，不得把状态 badge 置为绿色 `Connected`；`Endpoint test` 必须让用户输入一个来自 available models 的具体 model id，执行最小生成 probe，只有该 probe 成功才显示绿色 `Connected`。
...
- LLM Roles 的 Advanced Model Bundles 作为 pinned Model Group section 显示在普通 Available Models 之前；bundle authoring 仍保存 exact `route_id` fallback chain，并复用普通 Model Group drag/drop 路径。普通 Model Group 拖入 role 不需要先创建 bundle。
- LLM Roles 的 Advanced Model Bundles authoring surface 必须复用 Role 区域的 `Card` 层级和 Model Group 行的 `Item` 层级；它只是高级 authoring 区，不允许换成独立的小卡片视觉。右侧 pinned bundle 继续复用普通 Available Models 的 Model Group card 样式。
