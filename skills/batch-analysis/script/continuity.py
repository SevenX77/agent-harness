...
def check_continuity(context: dict) -> str:
    ...
    for char_id, prev_state in latest_states.items():
        ...
        prev_appearance = prev_state.get('appearance', prev_state.get('physical_state', '')) if isinstance(prev_state, dict) else prev_state
        ...
        prev_status = prev_state.get('status', prev_state.get('life_status', '')) if isinstance(prev_state, dict) else ''
...
def log_continuity_warning(entity_id: str, field: str, expected: str, actual: str, context: dict) -> str:
