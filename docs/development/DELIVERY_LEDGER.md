...
| P3b | 预测型探针改走生产工厂 | ⏳ 待办(裁决已下) | **放行(用户 2026-08-11:「动 ui 没关系,合理就行」),决议 D8 已记。** 判据是「探到什么 = 生产会发什么」,所以探针不再自己拼 HTTP,改走 `RouteChatModelFactory` 造模型问 1 个 token(与 `call/pre_call_probe.py::build_probe_model` 同款)。失败从此是 provider SDK 异常而不是 `httpx.Response`,判据换成 `classify_exception`,**`ProviderProbeStatus` 词表随之改,前端类型与展示同期跟上**——映射表在实施 PR 内给出 |
