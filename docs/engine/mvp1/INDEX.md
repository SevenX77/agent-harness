...
| **U10** | **API 操作面(engine↔studio)** | `ARCH1`(C 层) | API 契约切面→`03-api-contract`◆ · 入口实现→`07-runtime` · 事件供给→`02-observability` · 形状供给→`data-contracts`(RunResult) | `runner.py:run_skill:376`/`predict_skill:163`(live) · `compiler.py:compile_skill:41` | drafted(内容成段,可锁候选) | `03-api-contract` 17 块已 consolidate(§2.1/§3/§3.1-3.3)、`07-runtime` 顶层契约已成文;**内容齐**,锁待 studio HTTP 路由协同 + B baseline 补深 + resume/golden/iterate impl(归 kiro) |
...
| 模块 | W | B | A | 缺口 / 证据 |
|---|---|---|---|---|
| `01-contract/01-physical-layout` | ✅ `mvp1-alignment.md:115,123-126` | ✅ `baseline.md:6,12`; code `loader.py:146`, `runner.py:376/527/541`, `emit.py:15` | ✅ `mvp1-alignment.md:115,123-138`(.workspace 户型成段;golden/test_inputs SDK 未落地=impl 归 kiro) | `.workspace` 户型已写;`evaluate_golden_baseline`/`golden`/`test_inputs` Engine SDK 未落地(impl)。 |
...
| `02-mechanism/07-runtime` | ✅ `mvp1-alignment.md:23,29-30` | ✅ `baseline.md:10,20-29`; code `runner.py:163/376/623`, `__init__.py` public surface, `test_public_api_contract.py:131` | ✅ `mvp1-alignment.md`(§2/§3 顶层入口 + bootstrap + public API 契约成文;run/predict=模式) | 内容齐;public API live+干净,bootstrap 文档化 + 死簇清理归 kiro。 |
| `03-api-contract` | ✅ `mvp1-alignment.md:44,50-51` | ✅ `baseline.md`(B 成段 2026-06-06:入口/返回/端点 live·桩状态,12/13 live·resume 501;code `runner.py:163/376`/`compiler.py:41`/`result.py:68`) | ✅ `mvp1-alignment.md`(17 块迁移源 consolidate:执行签名/返回/事件/HTTP 端点总索引(13)/Golden/Iterate-Resume/Compile API 面全成段) | A 成段;B baseline 待补深;U10 可锁待 studio HTTP 协同 + resume/golden/iterate impl(归 kiro)。 |
