...
4.  **Identify the Error Category**:
    ...
    *   *Predict Mock / finish_task Shape*: If the failure appears only in Predict with custom mocks on an agent node, check the P1 mock format in `[[KB-08-predict]]` (`finish_task` is built in — declaring it in `tools` is itself compile error `[F-v3-agent-tool-reserved]`). The mock must be a `{phase_name: output_object}` JSON payload; Predict wraps it into the `finish_task` tool call and `## item-1` fenced JSON internally.
