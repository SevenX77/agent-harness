...
| **`GraphCanvas/canvas-authoring.ts`**（KEEP-MAIN） | D8 写盘侧同冲突 | wave3 的 `io:`/`path:` schema + extraFiles/.py 桩生成建在废弃 schema 上，不能单独嫁接。整体 KEEP-MAIN。若将来要"新建逻辑阶段自动生成 action 桩" UX，须在 main 的 mode+XML schema 上**重写**，不搬 wave3。 |
