...
def _md_to_models(md_text, schema):
    ...
    for blk in parse_md(md_text or "", schema):
        try:
            ...
            logger.warning("settings md block dropped (%s): %s", schema.__name__, exc)
    return items
...
def validate(output: dict, state_slice: dict, **kwargs) -> dict:
    ...
    # 1. Parse settings
    setting_items = (
        _md_to_models(raw_markdown, Setting) if raw_markdown.strip() else []
    )
    ...
    segmentation = state_slice.get("chapter_segmentation", {})
    ...
    return {
        "chapter_event_timeline": event_timeline,
    }
