...
- The parent-child bridge starts from the outer edge of the parent subgraph `+/-` point and targets the expanded child graph frame as a visual-only dashed line. It must not be a functional React Flow edge and must not target a child phase/global handle. The bridge belongs to the subgraph toggle component DOM, not a canvas-level SVG overlay with projected coordinates; layout keeps the group frame at the same fixed span as the component bridge. The bridge and subgraph ownership frame must use the same CSS dashed-border treatment so the dash weight, opacity, and cadence match exactly.
