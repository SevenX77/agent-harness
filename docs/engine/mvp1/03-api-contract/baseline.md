...
doc: baseline
status: drafted（B 成段:engine 入口/返回/端点现状对真实代码;WS-E3 P0-1 已暴露 RunResult.diagnostics;WS-E3 P0-2 已暴露 engine-side error catalog export;字段级形状链 owner 模块不复述）
---
...
> **现状一句话**:三接口面**主体 live**——执行入口(`run_skill`/`predict_skill`/`compile_skill`)、事件流(`event_subscriber`+`trace.jsonl`+WS)、studio 13 端点中 12 live;**唯一 501 桩 = resume**(`runs.py:69`)。WS-E3 P0-1 已让 run/predict `RunResult` 暴露 `diagnostics` 最终快照;WS-E3 P0-2 已在 engine 内提供 `export_error_catalog()` / `export_error_metadata(code)` 作为错误目录 SSOT，但没有实现 Studio HTTP `GET /errors` route。golden 逐节点 / iterate loop·图级 / V4 trace 增补 = target(归各 owner)。
...
- **Studio `GET /errors` route / golden 逐节点 / iterate loop·图级 / V4 trace 增补** = target(归后续 Studio thin route / 06-golden-eval / 02-iterate / 02-observability)。P0-1 的 `details` + `diagnostics` 已 live；P0-2 的 engine-side catalog export 已 live。
...
> **验"是否按 mvp1 改了"**:① resume 从 501 → 真实 checkpoint 寻址续跑;② Studio `GET /errors` 可枚举码表且薄透传 engine export;③ `DiagnosticEmittedEvent` 实时诊断事件 live。`RunResult.diagnostics` 列表已由 WS-E3 P0-1 完成；engine-side catalog export 已由 WS-E3 P0-2 完成。
