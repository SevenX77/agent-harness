...
- **语言分工(2026-09-03 用户裁决)。** ①**权威文档一律用英文写**:规则文件、设计、规格、契约、决议、台账、
  工单与验收判据——凡是被 agent 或门禁当作事实源读取的文档都算。逐字引用保留原文语种,旁附英译。自本条
  起新写或实质重写的文档立即适用;既有中文文档不为翻译单独开工单,在下一次实质重写时转换。②**给用户的
  汇报一律用中文**,采用自然、通俗的语言;避免生硬地堆砌技术术语或罗列大量纯代码块;若必须引入专业术语,
  须给出易于理解的通俗解释(四条机械判据见下一条「汇报可读性」),确保人机协作透明且高效。
- **权威文档干净上下文执笔(2026-09-03 用户裁决;是全局写作规范第 7 条在本仓的实例)。** 权威文档正文由
  **没有继承本会话历史**的独立执笔 agent,按一份**自包含 brief**(目标、范围、术语、已核事实与坐标、输出路径、
  边界、验收判据)一次写成;协调方不执笔,也不把多个片段拼成正文。返修只告诉执笔席"正文必须写成什么",
  执笔席**不得把返修过程写进正文**;返修累积到第二轮之后,改由**新起的**执笔 agent 按更新后的 brief 重写,
  不让同一席位越写越像会议纪要。判据:正文每一句都是正向陈述——"X 是什么、负责什么、怎么用";凡是只有
  知道了某场讨论、某个旧版本或某轮审查才读得通的句子(「…是要求,但不占北极星席位」「首版把它编在…,现移到…」
  「必须先纠正本文首版的假前提」这类),都是**语境泄漏**:要么改写成正向陈述,要么移进修订记录。上位文档
  自己写成辩论体时,**不照抄它的框架**,只取它的结论写成正向句。发前机械扫一遍正文:`但不`、`不是…而是`、
  `首版`、`原先`、`纠正`、`现移`、`不再`、`不替`——每处命中要么改掉,要么在报告里逐条说明为什么留。
  **Why**:2026-09-03 引擎仓北极星文档把上位决议的"排除项"辩论框架与自己四轮返修的历史都写进了正文,读者
  读到「是要求,但不占北极星席位」时不知道它在反驳谁。
- **汇报可读性(2026-09-03 用户裁决,教训:"每个字都认识,串起来看不懂")。** 汇报对象是产品负责人,
  发出前按四条自检,任一条不过就重写:①不用会话内自造的代号(临时编的编号、缩写、阶段名,如
  "D3""阶段 0c""r2"),指代一律用对象的完整名字,确需编号则同一条消息里先定义再使用;②术语首次
  出现即解释,工具名、库名、协议名、架构词都算术语,同一条消息里没解释过的术语不能用;③一句话只
  承载一个判断,推理链按步骤分行,先说要决定的是什么问题、再给答案、再给依据;④每条消息自带前提,
  不默认读者记得前文,引用前面裁过的事或查过的数要重述结论并给坐标。
