...
1. **mvp0 只允许说"契约已迁"**:
   ...
   - **不可说"mvp0/engine 迁移整域可 deprecated"**:`_migration-src/`→`02-mechanism` 只剩 `api-engine-studio-contract`(17 块,牵 studio,defer)未迁;`09-golden-eval`(baseline 已迁、alignment 待退役确认)、`11-io-and-edge-ops`、`records/state-checkpoint-storage-model`、`records/uncovered-areas`(约束/D-test 已分发,codex 复审)已清(见 §3.2)。
2. **锁迁移序(真实版)**:
