...
| P3a | 钉住真实调用发的请求 + 更正 P2c-2 的预算字段 | 🚧 待合并 | **P2c-2 判断错了并已更正**:我引 `dispatch.py:239` 断言生产发 `max_tokens`,但那条路**生产走不到**——`RouteChatModelFactory.build` 对四个协议各有分支返回原生 LangChain 模型,`GenericRouteChatModel` 只在兜底里构造,而 protocol 是那四个值的 Literal。实测真实对象的 `_get_request_payload`:openai/ark 一律 `max_completion_tokens`+`reasoning_effort`(deepseek 路由的 `PatchedChatDeepSeek` 也是),anthropic 是 `max_tokens`+`output_config.effort` 且无 `thinking` 块,google 是 `max_output_tokens`+`thinking_level`。新增 `test_production_wire_contract.py` 把这张表钉死;探针改回 `max_completion_tokens`(顺带修掉比我更早的 deepseek 错)。教训:**「生产怎么做」必须实测那个真会跑的对象**——`dispatch.py` 名字齐、测试齐、1026 行,唯独没人调 |
| P3b | 预测型探针改走生产工厂 | ⏳ 待办 | 原 P3「生产切到 dialect」不成立:生产不发 HTTP 请求体,它造 LangChain 模型对象。方向反过来——A2(`probe_provider_route`)必须用同一个注入的 `RouteChatModelFactory`,与 `pre_call_probe.build_probe_model` 同款,天然逐字节同源、不需要 dialect;A1/A3 是**发现型**探针(生产从不列模型、也不会去试它当前不用的方法),自己发 HTTP,dialect 正是为它们存在。「探测=生产」由共用工厂给,不由共用方言给 |
| P3c | 死路径处置(结构性,待用户拍板) | ⏸ 待裁决 | `call/dispatch.py` 六个 `_call_*` + `GenericRouteChatModel` + `LLMClientManager` 的 SDK 客户端构造对合法路由全不可达(约 1000+ 行)。按仓规该同期删除,但属跨模块结构性删除,且 `LLMClientManager` 另有活职责(熔断标记/用量,`chat_model.py` 在用)。先摆事实不动手 |
