...
doc: mvp1-alignment
status: audited-ready（**U1 单元锁定 2026-06-05**;子图绝对 path 解析 + DI 协议形状(契约)已定、删 registry;默认实现 LocalWorkspaceResolver 函数体归 kiro;文件未 FROZEN）
...
> 反转说明:早期模型用逻辑 id + `resolve_skill(id)` 查注册表,**已废弃**。mvp1 子图直接写绝对 path(见 `skill-syntax` §2.1),不再有注册表寻址、id→路径 映射、dotted-id、多命中歧义(path 是确定地址、无歧义)。

## 2. 数据流 / 机制
1. loader 编译 SUBGRAPH 节点 / agent `subgraphs[]` 时,读到 `path`(绝对路径)。
2. **边界校验**:path 必须落在 **copilot / host 工作目录边界**内(host 注入边界)——防引用工作区外的任意目录;越界 → 解析失败(报警;studio 标红 → 可触发"把该文件夹加入工作区")。
...
> path 绝对路径(PM 2026-06-05):path 写绝对路径——要能"随便放哪里"(相对路径绑死基准、做不到)。

## 5. 决策 + 动机
| ID | 决策 | 动机 |
|---|---|---|
| RS1 | 子图按**绝对 path 直接解析**,删 registry / id 寻址 / dotted-id / 多命中歧义 | path 即物理地址、随便放、直接打开,无需注册表 |
...
## 6. 测试关键点
1. 绝对 path → 子图 root;含 `GRAPH.md` 校验通过。
2. path 在工作目录边界外 → 解析失败、报警。
3. 递归子图防环。
4. 写旧的逻辑 id / `target_skill` → 应失败(已无 registry 寻址)。
