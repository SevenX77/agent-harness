...
| A16 | 逐模块核 06 error classification。 | `06-orch-error-classification/*`; `registry/error_classification.py`; `client_manager.py` | 修正当前函数签名、字段、fallback/fail-fast 映射;401/402/403/404 与 capability 类错误按当前语义记录。 |
| A17 | 逐模块核 07 fallback/circuit/probe。 | `07-orch-fallback-circuit-probe/*`; `gateway_chat_model.py`; `client_manager.py`; `llm_health_store.py`; `copilot_test.py` | 拆清 `_generate` 编排段、probe、down-cache、usage/event;保留 ChatX 瞬时 retry 是目标,当前 OpenAI/Anthropic client 仍 `max_retries=0`。 |
...
| A19 | 逐模块核 09 invocation runtime。 | `09-inv-invocation-runtime/*`; `gateway_chat_model.py`; `client_manager.py`; `models.py` | 标清当前自研 `_call_*`/dict response/`_coerce_text` 仍存在;目标是原生 ChatX invoke、保留 content blocks、usage/metadata bridge。 |
| A20 | 逐模块核 10 route chat model factory。 | `10-inv-route-chat-model-factory/*`; `models.py`; `resolver.py`; `client_manager.py` | 修正当前没有 `RouteChatModelFactory` 源码;职责暂由 resolver + client_manager 承担;Ark 当前仍走官方 SDK,目标才是 OpenAI-compatible ChatX。 |
| A21 | 逐模块核 11 provider profiles。 | `11-inv-provider-profiles/*`; `profile_selector.py`; `VerifiedProfile`; `client_manager.py` | 修正当前没有 `ProviderProfile` 调用层注册表;当前存在的是 `VerifiedProfile` / `select_verified_profile` 和 scattered provider kwargs;`ProviderProfile` 是 MVP1 target。 |
| A22 | 逐模块核 13 tracing/events/exceptions。 | `13-x-tracing-events-exceptions/*`; `events.py`; `exceptions.py`; `tracing.py`; `gateway_chat_model.py` | 标清当前 fallback event / exception / tracing helper 字段和触发点;事件 code 当前仍复用 all-providers-failed 系列,target 才拆专用语义。 |
| A23 | 核 predict migration 单独文档。 | `predict-migration-to-engine.md`; `predict_interception.py`; `protocol.py`; `resolver.py`; `predictor.py` | 修正当前 gateway 仍有 `PredictGatewayChatModel`、`PredictContext`、resolver predict 特判;目标才是 mock/path diff 业务移交 engine。 |
