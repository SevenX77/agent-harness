...
| P2b | `dialect/` 域落地,官方方法探测(A3)切过去 | 🚧 待合并 | 十个官方方法的方言收进四个模块(anthropic / openai 三种 / google),按 wire family 分家、per-method 差异做成构造参数(密钥放哪、内容要不要块、预算字段叫什么、怎么说思考多少)。`dialect/` 是**叶子域,不依赖网关里任何东西**——base_url 由调用方先过 catalog transform 再交进来,反向 import 会做成包级循环(同订正 6)。11 分支 if 链换成一张表,查不到就在发请求之前拒绝。88 条基线一行未动。顺带记下三处:anthropic 的 transform 探测侧原来没应用(新写法对所有基线用例同解)、openrouter 原来会先发一个错方言的请求再拒绝、A2 因为还在按 backend 名猜 wire 所以暂留两份重复规则(见决议订正 8-11)|
| P2c | A1/A2 归位 + 抽 `judge` + `probing/` 域 | ⏳ 待办 | 三件事同源:模型列表(A1)与路由生成(A2)现在都按 backend 名挑 wire,要先让它们说清自己在测哪个 call method,才谈得上共用方言;`endpoint_probe_backend` 的主机名猜测随之删除,改由 catalog 的 `protocol_defaults`/`host_overrides` 回答;`_probe_status` 与几个响应读取函数抽成 `judge`;`provider_probe.py`/`probe_contracts.py` 搬进 `probing/`,`_AWAITING_REHOME` 清空。**先给 A1/A2 补基线再动**(P2a 只覆盖了 A3) |
