...
| **实测答案 → 能力** | `measured_effort_capability(levels) -> CapabilityValue`（`registry/capabilities.py:205-221`）与 `measured_image_input(*, accepted) -> dict[str, CapabilityValue]`（`:224-256`）。**问出来的答案怎么变成能力，是本模块的事**——宿主只负责发请求和存结果，不自己拼 `source="probed_verified"` 的值。两个函数都**正反两个答案都写**：能力位空着读作"还没人问过"，会招来同一笔重复开销；而 catalog 的 `provider_doc` 声称若不被实测覆盖就一直站着。`measured_image_input` 的落点**不对称**：两个答案都写 `vision`（认不认图的是非答案，角色 lint 读它），**只有正答**写 `input_modalities`——模态清单是**下界**（宿主会把文档声称与已验证 profile 的清单并起来，见 studio `_merge_profile_capabilities`），正答往里加 `image` 撑得住，负答却不能把它改写成完备清单：一次被拒的图证明不了文档列的其他模态。上界属于 `vision`。 |
...
| **模型知识 → ③b（三项已下沉，notable 待下沉）** | `project_model_group_identity(route, endpoint, provider_label) -> ModelGroupIdentityProjection`（组 key/组展示名/section/owner_source/剥离 token，`registry/model_naming.py`）；`project_model_identity(...) -> ModelIdentityProjection`（展示名/section/置信度/owner_source/unknown_tokens，同文件）；`elect_model_group_section(projections) -> str`（组级 section 选举：按 `owner_source` 分层——`model_id` > `declared_vendor` > `endpoint_context`，同层多数、平票字典序；声明压过猜测在组级同样成立，2026-08-13 决议二，同文件）；`route_effective_capabilities(route) -> dict`（静态 + ready verified 合并，`registry/capabilities.py`）；`notable_model_ids(provider, notes_dir) -> list[str]`（仍在 `apps/studio/backend/app/services/llm_notable_models.py`）。**输出 = 模型知识 DTO**；③b **看得到**"route + endpoint 原始信息"（通用），**看不到**"family 折叠态/弃用区/展示名样式覆盖/数据源在哪个文件"（③a 应用加工）。 |
...
- **status**：已实现——Studio registry response 已输出 model_groups（`apps/studio/backend/app/routers/llm.py:1361-1383`）。分组内核下沉 gateway；family 折叠/弃用区展示留 ③a = target。**组级 section 选举也已下沉（2026-08-13 决议二）**：`elect_model_group_section` 按 `owner_source` 分层选举（声明压过猜测），Studio 后端/前端各自的厂商 if-链与等权多数票同变更删除，`section_label` 在 API 契约改为必填。
- **归属**：**③b 公共能力内核（已下沉 2026-08-13 #772）** `packages/graph-agent-gateway/src/graph_agent_gateway/registry/model_naming.py`（分组 + 组级 section 选举）。**③a 应用加工（留 studio / 前端）**：展示名样式覆盖、family 折叠、弃用区分区。
