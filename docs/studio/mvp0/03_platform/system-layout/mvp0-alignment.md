...
- **与 Trace-Visualization 协同**:
  底部抽屉弹起是由 [Trace 特性](../../trace-inspector/mvp0-alignment.md) 收到了第一条 Engine 回传事件而发起的强制展开行为。
- **与 Editor 区域占比分配**:
  Layout 为 [multi-file-editor](../../asset-explorer/mvp0-alignment.md) 锁定了左侧与中央区域的宽度比例，保障了 V2.1 目录型代码文件的展示空间充足。这两者的结合让多文件无缝协同成为可能。