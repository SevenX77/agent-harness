...
| T2.2 | Tier 2 | B | LOGIC Action 返回 key 契约校验 | `packages/graph-agent/src/graph_agent/core/loader.py`; `packages/graph-agent/src/graph_agent/core/graph_assembler.py:107-115`; `packages/graph-agent/tests/core/test_v21_actions_keys.py` | `loader.py` 用 `ast.NodeVisitor` 扫 action 源码 `ast.Return` 字面值 dict 提取 keys，未声明在加载期抛 `[F-v21-actions-keys]`，含 action file:line + key 名；动态 dict 由 `_build_logic_node` 闭包运行时兜底抛同 FATAL；单测覆盖静态 + 动态两条路径；`pytest packages/graph-agent/tests/core/test_v21_actions_keys.py -q` 通过 | M | T2.1 |
| T2.3 | Tier 2 | C | Canvas fan-out / fan-in fixture 验收 | `packages/graph-agent/tests/core/test_v21_graph_assembly_fanout.py`; `packages/graph-agent/tests/fixtures/fake_canvas_fanout/` | fake Canvas GRAPH 覆盖一出多、多入汇聚；执行断言 `{"a_out":1,"b_out":2}` 合并成功；冲突 fixture 抛 `[F-v21-state-conflict]`; `pytest packages/graph-agent/tests/core/test_v21_graph_assembly_fanout.py -q` 通过 | M | T1.2 |
...
**背景**: 对齐 R1.2/R3.1 与 design §3/§6。当前 `graph_assembler.py:109-115` 中 LOGIC action 返回 dict 后直接 `data.update(result)`，未声明 key 会静默污染黑板；design §3 已明确 AST 静态扫描为主路径，runtime 只做动态 dict 兜底。

**改动文件 + file:line**: `packages/graph-agent/src/graph_agent/core/loader.py` 负责加载期 AST 主路径；`packages/graph-agent/src/graph_agent/core/graph_assembler.py:107-115` 做 runtime fallback；新增/更新 `packages/graph-agent/tests/core/test_v21_actions_keys.py`。

**实施步骤**: 1. test-first 写静态拦截与动态 fallback 两组用例先 fail：字面值 `return {"undeclared_key": v}` 应加载期 FATAL，`return {**other, "new": v}` 应运行时 FATAL。2. 参考 `purity.py` 的 AST 扫描风格，在 `loader.py` 实施 `ast.NodeVisitor`，扫描 action 源码所有 `ast.Return`，提取字面值 dict keys 并与 outputs schema properties 比对。3. 在 `_build_logic_node` 闭包加入 runtime 兜底，捕获 AST 推不出的动态 dict key。4. 跑 `pytest packages/graph-agent/tests/core/test_v21_actions_keys.py -q`，验证两条路径都抛 `[F-v21-actions-keys]`。

**验收标准**: R3.1 undeclared key fixture 静态路径在加载期被 FATAL catch，动态 dict 由运行时兜底 catch；错误含 action file:line + key 名；声明 key 正例通过。

**风险 + Fallback**: AST 推不出的动态 dict（例如 `return {**other, "new": v}` 或 `result[k] = v; return result`）由 runtime fallback 兜底，不会漏拦；不引入宽松模式。
...
并行机会：T1.1 完成后，T1.2 fan-out 执行测试与 T2.1 schema key 提取可并行；T1.2 完成后，T2.1 schema 提取 + T2.3 Canvas fan-out fixture 可立即并行起步；T2.2 actions key 校验跟 T2.3 是正交并行（T2.3 只测 reducer，不依赖 actions schema）；T3.1 真正落地必须等 T2.3 证明 reducer 行为；T3.2 可在 T2.2 FATAL code 稳定后提前准备 xfail 标记，最终等 T3.1 确认 reference skill 不属于 xfail。
