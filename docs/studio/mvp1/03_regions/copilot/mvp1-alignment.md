...
> **Tier**: region | **Owns**: `copilot-session-persistence` 的 UI/session 渲染切面；`copilot-sdk-test-parity` 的配置结果消费切面 | **现状**: 面板与 WS live；session/window persistence live；ThinkingBlock/@mention/analysis bar 未落。 | **Related**: [baseline](./baseline.md)（双向）· `copilot-assist` · `studio-settings` · `settings` · `native-fs` · `golden-eval`
...
- 约束(2026-07-02 R3,2026-07-05 修订):连接状态**不占独立一行**——连接正常(open)时什么都不显示;仅断连/重连时在标题旁显示紧凑提示 chip(含 retry 倒计时)。会话 tab 条 = 单行横向滚动(本地 ScrollArea,绝不出现系统滚动条控件),且 panel 打开时始终可见;没有真实打开 session 时显示一个不落盘的临时 `Chat 1` 草稿 tab,保证右侧 `+` 动作菜单可达。**每个真实 tab 带关闭按钮**:关闭 = 从当前窗口状态移除该 tab,不删除其落盘 transcript;关到最后一个时把 `_window.json` 写成 `openSessionIds=[]` / `activeSessionId=null`,不创建新的空 transcript;临时草稿只在用户首条发送时物化为真实 session 文件。关闭当前活跃 tab 时激活其前一个邻居。`+` 是动作菜单: `New chat` 是显式创建并持久化空 session,`Restore chat` 打开原生文件选择器并默认指向本 skill 的 `.workspace/copilot/sessions/<skillId>` 目录。窗口状态由 `_window.json` 记录 `openSessionIds` + `activeSessionId`,打开 skill 只恢复该状态中的 tab,历史 session 文件只能通过 Restore chat 手动加回。
...
- 约束(2026-07-02 R5-E 图标 + 开关,PM「找一个类似 m 的星座图标」「把 copilot 开关换成圆形 m 图标放在 canvas 上,点击展开面板做过渡动画,再点收起」):身份标 = **仙后座(Cassiopeia)五星连线**自绘 SVG(`components/copilot/moirai-mark.tsx`,`currentColor` 主题化),一形三读——字母 **M**、**星座**、**节点 + 边的图(DAG)**。面板开关**从顶栏移到画布**(顶栏原 `Sparkles` 删除),面板收起时画布上浮现圆形 FAB(`copilot-fab.tsx`),PM 二轮定型:**① 默认右上角**;**② 画布范围内可自由拖动**(pointer 拖拽 + 边界夹取,拖/点用位移阈值区分,位置本会话持久);**③ 小尺寸(36px)、无边框**;**④ 配色 = 面板同款**——底用 `--studio-canvas-surface`(与 copilot 面板一致)、标用 `--studio-canvas-accent-strong`(与 header 标一致、浅一号),即「把 header logo 摘到画布上」而非实心高亮块。开合动画 PM 四轮定型为**容器变形(container-transform)**:**开**——点 FAB → 一个面板色 surrogate 盒子沿 L 路径(先**垂直**、再**水平**)飞到 panel header logo 落点(`headerLogoTarget`),**到位后连续把自己撑大成整个面板矩形**(`panelRect`:left/top/width/height/border-radius,一段 WAAPI 560ms)——即「圆长成面板」,而非圆淡出 + 面板另淡入;PM 四轮微调节奏:**travel 略慢、grow 略快** → offset 分割靠后(travel 占 ~0.64,grow 段用 snappy-decel ease),标随之淡出、面板内容(`[&>*]` 只淡内容不淡 surface,handoff 无闪)淡入。**收**——PM 定「直接收到 button、不像开那样走一圈、更利落」→ **不走 L 路径**,面板矩形**斜着直接缩回 FAB 圆**,时长更短(300ms)、ease-in,标末段淡回。reduced-motion / 无 WAAPI 直接开合。**收起触发 = 点 header 上的 MoirAI logo 本身**(PM「收的按钮去掉,点 logo 收」——原 `PanelRightClose` 按钮删除;logo 即 collapse 控件,带「收起」tooltip)。身份标颜色统一用**浅一号** `--studio-canvas-accent-strong`(= `--primary` 兑 ~25% 白;PM「logo 的颜色浅一号」),仅供 MoirAI mark/wordmark(header + FAB + morph)用,不外溢到按钮/焦点环/边。纯几何(默认位/夹取/logo 落点/L 路径/tap 判定/`panelRect`)抽 `copilot-fab-geometry.ts` 并 TDD;morph 在 `copilot-panel-morph.tsx`,Workspace 以 morph state 令 FAB/morph/panel 三态互斥。
...
- 约束(2026-08-15 用户裁决,会话身份契约 = capability `copilot-assist` COPILOT_ASSIST-5):**每个 tab 一条独立后端 SDK 对话**——发送报文带该 tab 的 `session_id`,流式事件归属发起查询的 tab(切激活 tab 不串流),关闭 tab 同时经 `session-close` 结束其后端 client。此前 New chat 只换前端视图、消息注入正在跑的对话,为实测缺陷。
- 测试: closed websocket disables send; reconnect updates status; switching skills restores `_window.json` 中的 session window; closing a tab preserves transcript files and closing the last tab writes an empty window state; empty windows keep a temporary draft tab + `+` menu visible; `+ → Restore chat` defaults to the current skill session directory; a message sent from tab A lands in tab A's transcript even when tab B is active while events stream back; closing a tab fires the backend session-close call.
- Status: live.
...
## 6. 测试关键点
1. session UI: 现状为 顶部多 session tab 历史持久化,并用 `_window.json` 恢复上次打开的 tab 集合与活跃 tab；空窗口状态合法,首条消息才把临时草稿物化为真实 session。
2. analysis bar: baseline 现状为 旧 golden 入口/sonner 口径残留 ⚠️；目标为 predict/run 后输入框上方弹 analysis bar，确认后消失。
3. 下钻 skillId: 现状为 Workspace 以 `currentSkillId` 驱动 CopilotPanel/useCopilot；子图下钻时 copilot 上下文与 currentSkillId 一致。

## 7. 涉及 region / platform
`copilot-assist` · `studio-settings` · `settings` · `native-fs` · `golden-eval`

## 8. gaps / 报警
- ✅ session UI: 顶部多 session tab 历史持久化 + `_window.json` 窗口恢复已 live；空窗口状态合法,临时草稿首发才落盘。
- 🚨 analysis bar: 旧 golden 入口/sonner 口径残留 ⚠️；目标 predict/run 后输入框上方弹 analysis bar，确认后消失。
- ✅ 下钻 skillId: Workspace 以 `currentSkillId` 驱动 CopilotPanel/useCopilot,子图下钻时 copilot 上下文与 currentSkillId 一致。
