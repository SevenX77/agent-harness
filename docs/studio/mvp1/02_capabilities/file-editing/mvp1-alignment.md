...
| ID | 决策 | 动机 |
|---|---|---|
| FILE_EDITING-1 | 写路径 | 单元 `native-rust-writer`；**为什么**：所有本地写收口到 Rust 唯一写者(D12)，避免双写者并发冲突 |
| FILE_EDITING-2 | autosave | 单元 `native-rust-writer`；**为什么**：编辑器 save 走 Rust 文件命令、保留 expected-hash 冲突检测 |
