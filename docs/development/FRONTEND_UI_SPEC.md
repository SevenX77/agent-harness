...
- API Keys 的 `Available Endpoints` 格子（endpoint 标签）的视觉 + 可点性必须跟状态走，把「未测但可测」与「架构性不可用」两种灰分开：`untested`（已配 key/base_url、还没测）**只把文字改成可读前景色**（`text-foreground`）—— border/底色仍保持和其它中性格子一样的 muted 处理（`border-border/70` + `bg-muted/10`），**不要把边框提升到实色**（那会让它像 verified/failed 一样喧宾夺主）；这样它就和死格子的 `text-muted-foreground` 置灰分开了。它是**点击即测**的活入口——点它跑**和整卡 Test 同一套** get-models 流程、只 scoped 到这一条 endpoint（`onProbeEndpoint` → 复用 `runProviderGetModels({onlyEndpointId})`，**同一套 per-step toast**，不另起简版），`cursor-pointer` + `role="button"`；`verified`（绿）/ `failed`（红）同样可点复测。`protocol_unsupported` 是架构事实（同域名不服务此协议），格子本体 `cursor-not-allowed` 不可点，唯一动手入口是尾部显式 Re-probe 按钮（force、绕过半衰期门）。`not_configured`（缺 key/base_url）保持 muted 死态；`testing` 走边框流动动画、不可点。设计源见 `docs/studio/mvp1/01_workflows/00_settings-ux-spec.md` §4.2。
...
- **边上下文检查点 (Edge context dot)**: 只在 phase-to-phase 的 `contextEdge` 中点显示，用作黑板/trace 检查入口。`Input -> phase` 和 `phase -> Output` 这类 IO 边界边不显示中点圆点；边界上下文入口收敛到 Input / Output 节点本体，避免 IO 节点和阶段节点之间出现重复的小圆点。但 IO 边界边本体仍是普通可编辑 edge，必须支持右键 `Disconnect`、端点拖拽断开和端点重连。
