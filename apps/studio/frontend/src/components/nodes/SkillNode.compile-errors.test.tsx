...
// Canvas node compile-error projection (authoring N3 atom #4): the node badge's tooltip
// expands from a bare count to the per-error field · line — message list. The detail is
// asserted on the trigger's accessible name + native title (the same SSR-safe idiom the
// PropertiesPanel marker uses) because Radix portals the styled TooltipContent, which is
// not emitted by renderToStaticMarkup for a closed tooltip.
