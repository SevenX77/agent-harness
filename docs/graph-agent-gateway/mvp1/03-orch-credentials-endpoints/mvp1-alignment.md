...
- **status**：现散在前端(前端拆 / 前端给 id)+ migration `_stable_endpoint_id`(`llm_credentials.py:369`)= target；MVP1 要求拆分 + 协议匹配 + 测试 + 生成 canonical id 下沉 ③b 公共(详见文末「已实现 / 与 baseline 差异」#7)。
- **归属**：region/platform **③b** `packages/graph-agent-gateway`：endpoint 拆分 + canonical id 生成(待下沉)。**③a** `apps/studio/backend`：批量探测 job/HTTP 包装、draft import-apply 工作流、`_stable_endpoint_id`(现 migration 用，待并入 ③b canonical id)。**① 前端**：provider 卡录入、多 URL 行(只收集原始输入，不拆 / 不生成 id)。
...
- **status**：baseline 的 fingerprint 只清理尾斜杠；MVP1 要求 fingerprint 输入已经是 protocol canonical base_url(详见文末「已实现 / 与 baseline 差异」#5)= target。
