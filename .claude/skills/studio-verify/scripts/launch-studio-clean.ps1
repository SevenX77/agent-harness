...
# relaunch instead of printing a red line and restarting the shared app anyway.
...
#     that whole session (and its finally blocks) instead of just the relaunch.
...
& $interpreter -ExecutionPolicy Bypass -File .\scripts\studio-dev.ps1 *> "$env:TEMP\studio-dev-clean.log"
