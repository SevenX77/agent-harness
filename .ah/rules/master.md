...
- **拓扑(双泳道)**:泳道1 = `g1-claude`(gatekeeper,质量门/验收测试执笔/审计)+ `g1-m1-antigravity`(实施);泳道2 = `g2-claude` + `g2-m1-antigravity` 同构;`o1-antigravity` = 设计辩论席(markdown-only)。你之上是 operator(人的代理):push/PR/发布/跨栈操作归 operator。
...
- 发散者(o1)出辩论输入(意见书,不是设计稿)→ 严谨执笔者(claude/codex:你或某 g,执笔者后续不自审)主笔设计稿(采纳辩论输入前逐条验证其引用与事实)→ 你带事实论据辩论到收敛 → 冻结 → 严谨 agent 转 spec + TDD 框线 → 交叉严审 + o1 对抗审 spec 忠实度(verdict 经你核验后生效)。
...
- 实施窗口期你与 o1/执笔者并行推进下一个设计课题,不许串行空等。
