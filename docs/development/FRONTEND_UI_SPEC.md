...
- API Key 输入框必须始终使用 `type="text"`，避免触发浏览器/扩展密码管理器。已有密钥的 ProviderCard 隐藏态必须渲染与真实 key 等长的 mask 字符串，不把真实 key 作为 input 可见 value；点 Eye 显示后才渲染真实 key。新建 provider 的空输入不得 mask placeholder；用户正在录入的新 key 可继续用 CSS mask 隐藏输入内容。
...
- API Keys 的 official 与 third-party/custom provider 卡片都必须渲染 `Available Endpoints` 区域，展示当前卡实际可测 endpoint（现阶段前端 DTO 只有单 `base_url` 时展示单条；多 Base URL 契约落地后展示多条）。endpoint 标签展示 base URL / method 数 / 状态，使用本地 `Tag` + Radix `Tooltip`，tooltip 必须包含 provider、endpoint id、base URL、protocol、status、routes、profiles、verified method ids、request mapper ids、tool protocol、last test/message 等可行动详情；不要用原生 `title` 代替或叠加 Radix tooltip。注意：Ark/OpenAI/Anthropic 等 official provider 的“endpoint”是 base URL + provider protocol；多出来的是该 endpoint 下的 verified profiles / call methods（例如 Ark 的 `ark_chat`、`ark_responses`、`ark_anthropic_messages`），不要把 method 伪装成多个 endpoint。
...
- The parent-child bridge starts from the parent subgraph `+/-` point and targets the expanded child graph frame through dedicated hidden React Flow handles plus a non-interactive `subgraphBridge` visual edge. Do not draw this bridge as a DOM span or pass a guessed width through parent node data. It is not persisted to `GRAPH.md`, not a dependency/context edge, and must not target a child phase/global handle. The expanded group may be pushed to the right of the whole parent topology; the target handle's vertical offset is derived from the same layout geometry as the group frame, and React Flow must read the final endpoint positions from measured handles. The expanded group must be a React Flow child of the parent subgraph node (`parentId = parent node id`), and child topology nodes must be children of that group, so dragging the parent node moves the bridge, frame, child nodes, and child edges as one bound unit. The bridge and subgraph ownership frame must share the same canvas accent variables so weight, opacity, and interaction state match, whether the current visual treatment is solid or dashed.
