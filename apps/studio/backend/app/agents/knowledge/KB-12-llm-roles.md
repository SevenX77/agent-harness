...
| **LLMCallSettingsEvent Emitted** | Every answered call reports what became of each setting the user chose. | Read the event's `settings` list: each entry pairs the requested value with one verdict — `applied` (the answer itself shows it took effect), `sent` (nothing can confirm either way, which is most settings), `adjusted` (moved to fit what the route accepts), `unsupported` (this protocol's request has nowhere to put it), `rejected` (the provider refused it) or `ignored` (accepted, and the answer contradicts it). Defaults nobody chose are deliberately absent. |
