...
| ID | 决策 | 动机 |
|---|---|---|
| ENGINE-1 | resume | 单元 `debug-resume-checkpoint`（引擎契约；studio owner=debug-resume）；**为什么**：节点级 checkpoint/resume 归 engine `04-run-outer/03-checkpoint`，Studio 只引用 |
| ENGINE-2 | engine SSOT | 引用 `docs/engine/mvp1/` 各契约 SSOT(compile/run/golden/trace)；**为什么**：engine 是格式/错误码/机制权威，Studio 平台只消费不重定义(R1) |
| ENGINE-3 | golden/path/schema | 单元 `golden-per-agent-node`（引擎契约；studio owner=golden-eval）；**为什么**：golden 落点/per-node 模型/失效校验归 engine `physical-layout`+`golden-eval`，Studio 引用 |
