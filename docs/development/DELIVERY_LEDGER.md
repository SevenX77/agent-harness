...
| P4-E12 | **golden case 内容可自定义改写(write_golden_case)** | 🚧 实现完毕待门禁+PR;真实验证=用新工具把 exp-b-round7 修正版 golden(13 段版+两处边界修正)写入替换违反 P0 的 12 段版 | 用户裁决 2026-08-07:「golden应该是可以自定义改的,不是只有run的结果才可以当作golden」。设计源 golden-eval mvp1-alignment.md F6 本就写着「golden 本身随时可写……Run 的真实输出可作 golden 默认种子……在其上编辑」,但实现唯一写路 `set_golden_baseline_for_run` 永远取封存 run 的 node_outputs——代码落后设计。补服务层 `write_golden_case_content`(404/409 locked/422 case_not_found/422 空对象四条边界;只重写 case 文件并标 `origin=authored`+`authored_at`,run 血统与兄弟节点 case 不动;**locked 首次从声明变实效**)+ 写档工具 `write_golden_case`(审批, cli.md:23 "golden writes" 已覆盖;不加 HTTP 路由——UI 编辑入口按 F3 仍归 I/O 数据流方向, 无调用方的链路不留)。凭空建 baseline/追加新节点 case 显式不做, 归 I/O 方向裁决。决议文档 `.kiro/specs/studio-moirai-agent-system/decision-2026-08-07-golden-case-authoring.md`。TDD:服务 6 条+工具 4 条先红后绿 |
