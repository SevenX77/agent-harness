...
| P3b | 预测型探针改走生产工厂 | ⏸ 提案已落盘,待裁决 | 方案见决议 D8。四个实现问题已查清并给了建议解法(临时 `ResolvedRoute`、凭据走注入、归 `probing/`、判据换成 `classify_exception`)。**唯一外溢到 UI 的部分**:失败从 HTTP 响应变成 SDK 异常,studio 消费的 `ProviderProbeStatus` 词表要改并同步前端——这是它必须先裁决而不是直接开工的原因。同期需一并裁决:`anthropic_messages` 方言给探测加了 `thinking` 块,而真实调用不发(实测),用带 thinking 的问题验证不带 thinking 的调用,结论未必成立 |
| P3c | 不可达调用路径的处置 | ⏸ 待裁决(建议全删) | 方案见决议 D9。三选一:全删(约 1300 行,合仓规)/ 只删入口留库(不建议,仓规禁止预留路径)/ 押后。建议全删,理由是这次教训本身即证据——**没人调用但名字齐全、测试齐全的模块会被当成生产事实引用**,它已经骗过一次 |
...
| P3c | 删掉任何合法路由都走不到的那条调用路径 | 🚧 本期 | **裁决已下(用户 2026-08-11:「没用的就删」),决议 D9 已记。** 不可达是坐实的:`ResolvedRoute` 是 `extra="forbid"` 的 Pydantic 模型、`protocol` 字段是四值 Literal(`registry/schema.py:29`、`:522`),而 `call/factory.py` 对这四个值各有分支直接 return(`:63`/`:76`/`:86`),兜底 `:97-116` 构造的 `GenericRouteChatModel` 永远不被构造。删:`call/dispatch.py`(1026 行)、`call/models.py`(301 行)、工厂兜底分支、`LLMClientManager` 里只有 dispatch 用的部分(四个 `_get_*_client` / `_client_cache_key` / `reset_stats` / 它自己那份 `_resolve_api_key`),以及只测这条死路的测试。**剩下的类要改名**——它只剩熔断标记与用量记账,不再管理任何 client。逐条比对后才删测试:活规则却只有死路径测过的,改打真实路径,不顺手删 |
