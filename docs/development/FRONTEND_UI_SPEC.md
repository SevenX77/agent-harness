...
- Canvas chrome depth (shadows / inset highlights / canvas pane background / nested panel-card tints) is **theme-scoped**, defined once as `--studio-shadow-*` / `--studio-inset-*` / `--studio-canvas-pane` / `--studio-panel-card-bg*` variables in `index.css` (`:root` = light, `.dark` = dark). Light follows the shadcn light convention: surfaces separate via a visible `--border` plus soft layered low-alpha shadows, no white inset highlight, and the canvas pane uses muted gray so white cards/panels read against it; dark keeps heavy elevation shadows + faint white inset. Never write a literal dark-tuned `box-shadow` (`rgb(0 0 0 / …)` / `rgb(255 255 255 / …)`) directly on a chrome rule — it silently breaks the other theme(2026-07-02 亮色模式教训). `--studio-canvas-edge` is `var(--ring)` in light (light `--border` is near-invisible on the muted pane) and `var(--border)` in dark.
