...
- 机制补充(plan 的每个文件自带写入前提): 种子化写的是**活着的那个 baseline**,所以 plan 里的文件分两类——补空缺的 case 文件**当时不存在**,重填坏 case 与每次都要重写的 `baseline.json` **当时存在**。plan 因此**逐文件**声明它据以计算的前提:`GoldenBaselineFile.expected_hash` 为 `null` = plan 那一刻这个路径上什么都没有,写入按 create-if-absent 执行,期间冒出文件就冲突;有值 = plan 读到的正是这份内容(sha256 over LF-normalized text,与 native-fs 的 `workspace_text_hash` 同一个算法),写入按乐观锁替换,内容变了就冲突。**不允许无前提覆盖**:用户可能正在 I/O 面板改同一个 golden,静默清掉它恰好违背本条「有效 golden 不被自动覆盖」的承诺。
- 决策(前提属于 plan,不属于调用方): 前提由**算出这份内容的人**声明。plan 是唯一读过磁盘现状的一方,调用方(前端 client)只照着 plan 说的执行,不自己猜某个路径是新建还是替换——猜错的代价正是 2026-08-21 那个缺陷:seeding 沿用了保存 baseline 的 create-if-absent,而那个模式只在「每次新建一个 `baseline_id` 目录、文件天然不存在」的前提下成立,seeding 写进活 baseline 时前提不成立,于是重填坏 case 与 `baseline.json` 一律被拒,F6 在桌面 app 里**只补得了空缺、补不了破损**。两条写盘路径(保存 baseline / 种子化)因此收进同一个 writer。
- 测试: predict 提升返回 409;无/空/坏 golden 节点 Run 后 golden 默认以 run 输出填充且可编辑;有效 golden 不被自动覆盖(`tests/services/test_golden_seed_refills_unusable_cases.py` 九条:三种成因各一条、逐字节不动一条、写进现有 baseline 一条、case 记来源 run 一条、plan 不落盘一条、无事可做不写文件一条、plan 文件带着它读到的哈希一条)。前端一条断言 client 按每个文件自己的前提调用写盘(`src/api/client.test.ts`)。
