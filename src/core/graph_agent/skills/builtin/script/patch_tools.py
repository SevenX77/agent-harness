...
Context keys used:
    ...
    error_items         list[dict]      — failed raw item dicts (patch targets)
    ...
    added_items         list[dict]      — items added via add_missing_item
...
def apply_field_patch(item_id: str, field: str, value: Any, context: dict[str, Any]) -> str:
    ...
    Args:
        item_id: The `_md_id` value of the target item (matches the '## Header' text
    ...
    return f"OK: patched item _md_id={item_id!r} field={field!r} → {value!r}"
...
def add_missing_item(item_md: str, context: dict[str, Any]) -> str:
    ...
    """
    from story_forge.core.md_parser import parse_md  # local import to match skill pattern

    schema = context["schema"]
    try:
        new_items = parse_md(item_md, schema)
...
def finalize(context: dict[str, Any]) -> str:
    ...
    Data flow:
        ...
        error_items     — patches from context['patches'] applied via _md_id matching
        added_items     — new items parsed by add_missing_item

    final_results = valid_results + patched_error_items + added_items
    ...
    # Apply patches to error_items, matching by _md_id
    patched: list[dict[str, Any]] = []
    for item in error_items:
        item_id = item.get("_md_id")
        if item_id and item_id in patches:
            item = {**item, **patches[item_id]}
            patch_keys = list(patches[item_id].keys())
            logger.debug("finalize: applied patch for _md_id=%r fields=%s", item_id, patch_keys)
        patched.append(item)

    context["final_results"] = valid + patched + added
...
def validate(context: dict[str, Any]) -> tuple[bool, str | list[str]]:
    ...
    """
    from story_forge.core.md_parser import DiagnosticReport, diagnose  # local import
    ...
    report: DiagnosticReport = diagnose(final_results, schema)
