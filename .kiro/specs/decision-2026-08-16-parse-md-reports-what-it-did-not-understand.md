...
即:出口契约并列给出**两种整体形态** —— `prompt.py:65` 的 bullet 骨架
`- {name}: <值>`,以及 `prompt.py:67-69` 追加的那一句「也可以把整个对象写成一个
JSON 对象,放在 `## ` 标题下的 json 代码块里」(原文用三反引号 json 栅栏)。
**但两种形态都只说到"这里放一个值"为止:一个 `list[dict]` 字段,模型被告知的是
`- parsed_segments: <值>` 或"整个对象写成 JSON",没有任何一句话说这个 `<值>`
该长什么样。** 模型写嵌套 bullet 不是违规,是在填空。
...
4. **值形态仍然没有写进出口契约。** 模型看到的骨架给了两种整体形态
   (`- name: <值>` 与整个对象一块 JSON,`prompt.py:61-70`),两种都只说到
   "这里放一个值"为止,仍然不知道一个 `list[dict]` 字段该怎么写(§2.2 引文)。本 PR 让"写错了"这件事
...
裁决与论点不改;**写错了的事实陈述就地订正**,并在这里留痕,让下一个读者知道
哪一句被动过、动了什么。这与 §四"#848 的决议是一份有日期的裁决记录,不回头改"
不冲突:那说的是**不因后来的变化去追改当时正确的记述**,这里改的是**当时就不
准确的一句描述**。

- **2026-08-18(PR「the parse-gap tests pin delivery, not bookkeeping」)**:
  订正 §2.2 与 §六第 4 条对出口契约的描述。原文说模型"只被告知写
  `- parsed_segments: <值>`"/"看到的骨架只有 `- name: <值>`",漏掉了
  `_render_business_data_md_skeleton` 并列渲染的第二种形态 —— `prompt.py:67-69`
  追加的「也可以把整个对象写成一个 JSON 对象,放在 `## ` 标题下的 json 代码块里」。
  这一句由 #825(`f9a32905`)引入,早于本决议写作时的 HEAD `59e74020`,所以是
  **写作当时就已存在**的事实,不是后来才变的。§2.2 的代码引文本身是完整的
  (它包含了那三行 `lines.append`),不准确的只是紧随其后的那句散文小结。
  **论点不变**:两种形态都只说到"这里放一个值"为止,一个 `list[dict]` 字段
  该长什么样,契约里仍然一个字都没有。
