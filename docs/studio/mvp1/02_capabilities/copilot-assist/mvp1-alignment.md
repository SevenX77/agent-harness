...
- **决策 COPILOT_ASSIST-8(一次提名是用户挑出来的一个东西,不是一段被猜出来的上下文;2026-08-20 立,问题台账 CP1)**:
  ...
  - **提名的身份 = `(kind, ref)`,两者都由发出这条消息的前端给出**,后端不反查、不补全、不猜。
    ...
    唯一定位那个对象(file=工作区相对路径;phase=阶段 id,子图内的写成 `<subgraph>/<phase>`;
  ...
  - **`label` 只用于回显,不参与定位**。显示名会变(阶段改个名),而已经发出去的那条消息里,
  ...
  - **解析失败当场说,不静默丢**:取正文的两类,`ref` 解析不到(文件被删、阶段被改名)时,
...
- **决策 COPILOT_ASSIST-10(提名在屏幕上、在正文里、在菜单里各长什么样;2026-08-21 立,问题台账 CP1)**:
  ...
  - **③ 菜单按 ① 列的顺序分组:file → phase → dot → error → trace,每组至多 8 条,
    ...
    词表,用户想找一个阶段时不该在文件里翻;每组设上限,是因为一个大工程的文件数会把另外
  ...
  - **⑤「彩色 pill」= 一眼看出这不是打出来的字,不是给五个 kind 配五种色**。
    ...
    **颜色只表达严重度,不表达分类**,判据是「把一屏截图去饱和,信息不该丢失」。
    ...
    唯一会歧义的情形——同名的一个文件和一个阶段——由 pill 的 hover title `kind · ref` 消歧。
...
- **决策+动机**：**两入口**(新建 skill 时可选 + chat 说"帮我建个 X")与**产出判据**(合 FROZEN 骨架、
  ...
  **原方案是「copilot-SDK 调一个独立 brainstorming *graph* skill」,2026-08-22 实施时判定不成立**,
  ...
  2. **它在需要它的那一刻还跑不起来**。graph skill 的 agent 阶段要经 gateway 解析 **LLM role**;
...
| COPILOT_ASSIST-12 | 会话的文件系统配置面 = 只开 `project` | 单元 `copilot-assist` F6；**为什么**：`skills` 名单只是**过滤器**——它决定模型能叫哪些**已被发现**的技能，而"发现"来自 `project` 这个配置面（它才是让 CLI 去读 `<cwd>/.claude/skills/` 的那一项）。此前为了不让开发机的 `~/.claude` 漂进 copilot 而传了空列表，把两件事一起关掉：随包技能池明明已被物化进工作区、名单里也写着它，模型那边却一个都看不见，点名调用直接答 Unknown skill（2026-08-22 真机实测 brainstorming）。正确口径是**按会话种类分**：聊天会话 `["project"]`——`project` 指的是本次打开的**skill 工作区**，不是开发机；探针会话一个技能都不开，配置面整个关掉。两种都不含 `user`。**边界不靠它守**：`project` 会连带读进工作区自己的 `.claude/settings.json`，而执行类与写类的硬边界一律由 PreToolUse hook 承担（R8.2/R8.3，每次调用必触发，不受 allowed_tools 或配置面放行影响） |
