...
def format_events_raw(event_entries: list[dict]) -> str:
    """Format events as markdown text for settings stage."""
    lines = []
...
def validate(output: dict, state_slice: dict, **kwargs) -> dict:
    """Validate and process event review stage."""
    raw_events = output.get("parsed_events") or output.get("events") or []
    original_timeline = state_slice.get("_original_event_timeline")

    # Safe review fallback if parse failed or returned empty
    if not raw_events and original_timeline:
        logger.warning("Review parse returned 0 events, restored original timeline")
        events_raw = format_events_raw(original_timeline.get("events", []))
        return {
            "event_timeline": original_timeline,
            "event_entries": original_timeline.get("events", []),
    ...
    parsed_events = []
    ambiguity_reports = state_slice.get("_ambiguity_reports") or []
    if ambiguity_reports:
        ambiguity_reports = copy.deepcopy(ambiguity_reports)
    ...
    for e in raw_events:
        ...
        # Handle log_ambiguous_events
        if e.get("ambiguous") or e.get("ambiguity_reason"):
            event_id = f"{chapter_number:04d}{index:05d}{etype}"
            reason = e.get("ambiguity_reason") or "Review phase marked ambiguous"
            report = {
                "event_id": event_id,
                "reason": reason,
                "confidence": 0.5,
            ...
            # Deduplicate report by event_id
            ambiguity_reports = [
                r for r in ambiguity_reports if r.get("event_id") != event_id
            ]
    ...
    # Generate events_raw for the settings stage
    ...
    return {
        ...
        "_ambiguity_reports": ambiguity_reports,
        "events_raw": events_raw,
    }
