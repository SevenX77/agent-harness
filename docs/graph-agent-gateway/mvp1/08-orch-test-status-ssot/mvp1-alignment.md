...
binds_code: packages/graph-agent-gateway/src/graph_agent_gateway/registry/projection.py:ProviderModelStateProjection/project_route_state · packages/graph-agent-gateway/src/graph_agent_gateway/registry/catalog.py:ProbeCatalogStore/materialize_probe_catalog_candidates · apps/studio/backend/app/services/llm_credentials_evidence.py:merge_route_evidence · apps/studio/backend/app/services/community_catalog_sync.py:sync_verified_catalog · apps/studio/backend/app/services/community_catalog_runtime.py:sync_verified_community_catalog_into_credentials · packages/graph-agent-gateway/src/graph_agent_gateway/registry/schema.py:EvidenceRecord/ProviderImportDraft/ProbeResult
...
> **Status**：设计定稿（2026-06 重新定位：MVP1 不做 Import Draft 主线，`draft` 命名退役为 legacy；探测知识库改名为 Probe Knowledge Catalog）。代码已落地 canonical 入口,但**这三个名字全部换过**:模块 = `graph_agent_gateway.registry.catalog`(`probe_catalog.py` 别名壳已删),studio 侧按职责拆成 `community_catalog_sync.py` / `community_catalog_runtime.py` / `llm_credentials_evidence.py`,而 `llm_probe_catalog.json` **这个文件已退役**——`apps/studio/backend/app/services/runtime_truth_init.py:42-45` 写明三个 legacy catalog 文件不再在启动时创建,证据改住 credentials 的 `route.evidence`。`llm_import_drafts.py` 已删除,`ProviderImportDraft` 仍在 `registry/schema.py:466`；`snapshot_version` 填充仍由 loader/materializer/host 侧负责。
...
- **代码下沉/改名**:6 态投影内核与 Probe Knowledge Catalog canonical API 已进入 gateway 包；颜色渲染 / 存储介质 / 远端源配置 / 上传审批留 ③a。剩余兼容债只剩一半:`llm_import_drafts.py` 已删除,`ProviderImportDraft`(`registry/schema.py:466`)这个容器名仍未迁成正式 `ProbeKnowledgeCatalog` schema。
...
- Studio 侧的 catalog/证据入口现已按职责拆开,`llm_probe_catalog.py` 与 `llm_import_drafts.py` 两个文件都不在了:证据合并进路由 = `apps/studio/backend/app/services/llm_credentials_evidence.py:56` `merge_route_evidence`;远端 catalog 拉取与校验 = `apps/studio/backend/app/services/community_catalog_sync.py:146` `sync_verified_catalog`;把同步结果落进 credentials = `apps/studio/backend/app/services/community_catalog_runtime.py:89`。共享内核(draft/证据库/候选物化)在 gateway `registry/catalog.py`。
- `llm_import_drafts.py` 已删除;它承担的 draft/evidence store 内核在 gateway `packages/graph-agent-gateway/src/graph_agent_gateway/registry/catalog.py`(`ProbeCatalogStore:45`、`new_evidence_library:188`、`merge_evidence_library:382`)。Import Draft 非 MVP1,apply 路径不作为公开产品功能。
...
本 alignment 覆盖 08 brief 的全部要求:`registry/projection.py:project_route_state`(6 态投影)+ `registry/catalog.py`(Probe Knowledge Catalog canonical 入口;原文并列的 `llm_probe_catalog.py` 已按职责拆开,见上)两个核心对象已落到真实代码,并完成设计重新定位:6 态投影内核 = ③b、Probe Knowledge Catalog 内核 = ③b、Import Draft 非 MVP1 主线、`draft` 命名退役为 legacy。为说明"探测→持久化→投影→复用",额外引用 router、health store、role materializer 作为证据线索,均标注 ③a/③b 归属。
