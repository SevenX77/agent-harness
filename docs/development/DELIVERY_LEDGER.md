...
| P4-E9 | **终态出口共享产出契约提醒:get_run_detail 也要说话** | 随本行同 PR 交付 | exp-B round6 实测:P4-E8 把提醒挂在 `wait_for_run`, 但会话全程用 `get_run_detail` 轮询终态、从未调 `wait_for_run`——提醒一次都没送达。终态是同一个事实, agent 从哪条工具读到它, 提醒就在哪条工具出现:提醒文案提成共享常量, `get_run_detail` 在 success+零产物时同样附 `output_contract_reminder`(failed 等其余状态不附, 语义同 P4-E8)。决议文档 `.kiro/specs/studio-moirai-agent-system/decision-2026-08-07-run-terminal-output-contract-and-cli-read-tier.md` D-1 |
| P4-E10 | **CLI 免审批名单对齐 cli.md 读/探测档(第三次漂移)** | 随本行同 PR 交付 | 权威规则 cli.md:23「Read and probe tools are pre-allowed」, 但 lib.rs `CLAUDE_STUDIO_ALLOWED_TOOLS` 手抄名单连续两波工具上新未跟:纯读 `get_skill_output_contract`(#585)与 KB-13:50 归类为 probe 的 `run_role_test`/`test_llm_endpoint`/`test_llm_endpoint_models`/`probe_llm_route` 均漏。本次补齐 5 项;写/执行类照旧走审批。**观察项**:名单是 Rust 侧手抄清单, 已第三次漂移, 暂以「新工具 PR 必核 cli.md 档位并同步名单」为纪律, 不引入跨语言生成链路。决议 D-2 |
| P4-E11 | **pause_run/stop_run MCP 工具(能力对等)** | 随本行同 PR 交付 | round6 会话原话 "No cancel tool is exposed on this CLI surface — I'll let the doomed run expire"(A[223]):人有 Pause/Stop 按钮(P4-E6), copilot 没有。补两个工具直调 `run_manager.pause_run`/`stop_run`(与 HTTP 路由同链, gate 广播照发→状态对等免费);非法状态由 run_manager 409 语义落 is_error, 工具层不重复防御。执行档、不进免审批名单(与 run/resume 同待遇)。cli.md 执行类枚举 + KB-13 工具地图同步。决议 D-3 |
