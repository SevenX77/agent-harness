...
| E3 | golden 创建路A(copilot 协作):入口①trace 内占位节点旁按钮(Design golden,**不要求先有 run**——占位节点正是从没产出过东西的那种节点;**这个按钮由 trace 区域(`FocusedNodeActions`)渲染,不在 `TracePanel` 里面**:它属于「选中的这个节点能做什么」,与有没有 run 无关,而区域在一次 run 都没有时挂的是运行历史列表、根本不挂 `TracePanel`——放在 `TracePanel` 里等于在最需要它的那种 skill 上不存在;点它开一个新的 copilot chat,提示词按 [`golden-eval`](../02_capabilities/golden-eval/mvp1-alignment.md) §4 描述驱动那条写)②**run 跑完 copilot 输入框上方分析 bar 弹窗**(predict 跑完不弹——E8 规定 golden 非从 predict trace 捕获;确认→无 golden 节点自动写 golden;细化自旧 sonner 批量,裁决记录见 [`copilot-assist`](../02_capabilities/copilot-assist/mvp1-alignment.md) F7 修订 2026-08-29) | trace/copilot | live |
