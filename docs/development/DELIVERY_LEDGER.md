...
| P3 | 收口时定 `ignored`,每次调用发一条设置事件(步骤帧) | ✅ 已合并(#698);**真机验收(2026-08-10)** | `sent` 与 `applied` 必须分开:沉默不是证据,把「无法印证」报成「已生效」是撒谎,报成「没生效」是冤枉 |
