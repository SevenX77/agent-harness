...
- **D12 本地操作全量 Rust(唯一写者,🔒PM 裁定 2026-06-01)**> "全量切 rust, 除了 graph agent 和 llm gateway 相关使用 python sidecar, 其他本地操作都用 rust";skill 源文件 / `.workspace` / copilot patch 落盘 / 编辑器 save 全走 Rust 命令(copilot 模型 B:Rust 即时落盘 + 记 checkpoint,Reject 从 checkpoint 精确还原),Python 端点退为只读 + 编译装配。
