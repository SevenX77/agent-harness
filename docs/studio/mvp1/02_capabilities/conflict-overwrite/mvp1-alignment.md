...
- Routing input: **the canvas is handed the compile pass's diagnostics, not the per-node projection the badges use.** The projection answers "which ROOT node owns this file", and a phase inside a child skill answers `null` by design (`studio/diagnostic-paths.ts`) — correct for badges, because a nested phase has no node of its own on the root graph. Routing asks a different question — which chain of subgraphs leads to the preview child that should carry the popover — and the diagnostic's own path answers it. Projecting first therefore drops exactly the conflicts routing exists for; measured 2026-08-21 on a two-level fixture, where the diagnostic arrived complete and the canvas did nothing.
