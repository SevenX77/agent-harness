...
Status: live for local git snapshot list, revert, and WS-6 publish artifact history visibility. README-era ownership of run detail/batch summary is not reflected in mounted code.
...
| History DTO kind | Local history items classify `auto_run`, `manual`, `publish`, and `other`; messages starting `publish-artifact-` appear as `kind="publish"`. | `apps/studio/backend/app/models/git_history.py:10`, `apps/studio/backend/app/services/git_local.py:408` |
| Publish artifact record | Publish success writes an allow-empty local git commit named `publish-artifact-{artifact_id}` so `/history` can show the release snapshot. | `apps/studio/backend/app/routers/skills.py:326`, `apps/studio/backend/app/routers/skills.py:331` |
...
- Owns: local git snapshot list, selection, revert, run-autocommit visibility, publish artifact snapshot visibility.
- Ownership conflict: run detail and batch summary appear closer to `timeline`/`input` than local git history.

## Known Drift

- Workflow assigns successful-run autocommit to publish/save; Local History shows snapshots and now receives publish artifact commits, but still does not explain git status in detail (`apps/studio/frontend/src/components/history/HistoryPanel.tsx:67`).
...
- Real e2e is still deferred by current project strategy; local-history closeout is covered by backend route tests plus UI smoke.
