...
| **U5** | **checkpoint 嵌套拓扑 + 存储纪律** | `CK1-6`(records/state-checkpoint,2026-06-03) | base 切面→`03-checkpoint`◆ · 内层 messages 切面→`08-messages-state`◆ · 迭代切面→`02-iterate` · agent 切面→`01-agent-loop`(经 ns 挂) · 数据切面→`data-contracts`(data delta reducer 待补) | `graph_assembler.py:151`(compile checkpointer) · `state.py:214`(messages DeltaChannel) · `cognitive_flow.py:292`(interrupt 依赖 checkpoint) | drafted | 🚨 **权威模型仍半躺 `_migration-src/records/state-checkpoint-storage-model.md`**(已迁 7 块/未迁 7 块)→ 正式模块只摘要承载,递归拓扑/delta-compact 矩阵/durability 未全迁 |
| **U6** | **变更→失效模型** | records/change-invalidation(Task2 C3,2026-06-03) | 失效契约切面(A1–A5 轴+矩阵)→`05-invalidation`◆ · golden 切面(A2a 编译期硬错)→`06-golden-eval` · checkpoint 切面(A2/A4/A5 置灰)→`03-checkpoint` · cache 切面(源 hash)→`01-compile` | `compiler.py:38`(cache) · `diff_skill`(待实施) | drafted | 🚨 §1 轴定义已迁入 `05-invalidation`;但 golden-stale "编译期→eval 期"反转的等价承载**未完全核对**(records §2 标⚠️未迁) |
...
### 🔁 重复风险(同事实两处,需确认退役)
- **U5 / U6 的权威模型仍半躺 `_migration-src/records/`**:`state-checkpoint-storage-model.md`(7 迁/7 未迁)、`change-invalidation-model.md`(6 迁/1 未迁)——正式模块(`03-checkpoint`/`08-messages-state`/`05-invalidation`)部分只摘要承载。**风险**:records(留底)与正式模块(SSOT)对同一事实并存且不等价。**动作**:补迁未迁块 → records 降级为纯 workflow 留底(不再当实现 SSOT)。
