...
| P4-E6 | **跑起来就停不下来:补 cancel 语义 + Stop 按钮** | ✅ 随本行同 PR 合入 | PM 反馈"run 跑的时候按钮至少该能停"。查证:唯一能提前结束 run 的只有 `DELETE /{run_id}`, 它 terminate 进程后 `shutil.rmtree` 把整个 run 目录删掉——那是删除, 不是停止, "停下来看看跑到哪了"这句话在系统里无法表达。新增 `POST /{run_id}/cancel`:终止 worker, 然后走**和自然结束同一条** `_finalize_terminal_run`, 所以封存/同步/广播的路径完全一致。状态词表补 `cancelled` 并抽成 `RunStatus` 单一别名(前后端各一份)——run 的状态会流经 batch 视图, 只补一半会让自己的数据过不了校验。gate 广播补 `stopped`(不是 fail:用户主动停不是缺陷, 报 fail 会弹一个不存在的错误抽屉), 并把"状态→广播"改成显式映射表, 新状态不能再默默落进 fail。前端:`running` 阶段 Run 按钮换成 Stop(禁用的 Run 只说"等着", 不说怎么别等)。**已知缺口(未在本 PR 扩)**:被中途停掉的 run 从没封存过产物, 所以 `get_run_detail` 对它返回 `artifact.run_not_sealed`(409)——与 P4-E3 predict 那条同类, 需要把增量写入的 trace 纳入可读范围; 本 PR 的测试只断言 cancel 自己承诺的东西(worker 停、run 不被删、状态为 cancelled)。测试 2 后端 + 2 前端先红后绿;backend 1547 全绿, 前端 1909 全绿 |
