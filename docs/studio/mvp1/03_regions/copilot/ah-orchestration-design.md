...
**D3 · 生命周期:会话属于工作区,不属于面板(2026-08-03 PM 反馈后修正)。** 会话的持有者是**工作区**:
收起 copilot 面板只是把渲染器摘下来,会话与它的本地终端客户端都继续活着,重新展开时同一个会话原样回来
(期间的输出在重新 attach 时补齐)。**结束本地终端客户端**的只有三件事:切换到别的 skill、关闭 app 窗口、
以及 D4 的 Close;其中**只有 Close 才真正结束 ah 运行时**,前两者仍是 detach,ah 的 tmux 会话继续由 ahd
持有。这条的理由是:agent 任务动辄跑几十分钟,"收起一块面板"绝不能等于"杀掉一个正在工作的编队"。

工程上的推论(修正前的实现正是栽在这里):会话状态**不能**存在 copilot 面板组件里——面板每次收起都会被
卸载,存在那里的会话会随之消失,而它的终端客户端还留在后台没人回收。
...
**画布浮层一律让开侧边面板。** Compile / Predict / Run 这条动作栏属于画布,它在画布区域内居中——
即在左右两侧浮层之间的空档里居中,读的是与 minimap、画布控件同一组 `--studio-canvas-*-safe-area`
变量。按窗口居中会让它在 copilot 面板占据右侧时压到面板上(2026-08-03 PM 反馈)。
