...
1. **mvp0 只允许说"契约已迁"**:
   ...
   - **不可说"mvp0/engine 迁移整域可 deprecated"**:`_migration-src/`→`02-mechanism` 尚未清完,尤其 `11-io-and-edge-ops`、`api-engine-studio-contract`、`records/state-checkpoint-storage-model`、`records/uncovered-areas` 仍有未迁块;`09-golden-eval` baseline 已迁、alignment 旧决策 A 块待退役确认(见 §3.2)。
2. **锁迁移序(真实版)**:
   ...
   - 再清 **A 真空/摘要未迁**:`01-contract/02-skill-syntax` 的 resource/example、iterate、io 语法真空;~~`06-golden-eval` workspace 重写~~(✅ alignment 已反转重写,余 §8 实现级 gap+未经 codex 审);`03-api-contract` 完整表;`11-io-and-edge-ops` 归入 `graph-exec`/`observability`;checkpoint/messages records 深度。
