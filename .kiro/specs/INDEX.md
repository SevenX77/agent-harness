...
| [`studio-moirai-agent-system/`](./studio-moirai-agent-system/) | Draft v4 (2026-07-09 复审修订完成,待 PM 审) | **MoirAI 代理人体系(四层资产单源,双路装载)**:角色/手册/知识库/技能四层收拢 `app/prompts/` 随包 · MoirAI 从"分诊台"改代理人闭环(理解→research→计划→执行→择机派遣) · 知识库索引+13 篇主题文档取代开发仓 docs 挂载(打包版不再缺知识) · SDK 面板=MoirAI 化身+原生 AgentDefinition 派三女神 · 工具边界代码强制(读声明式放开/写白名单经 PreToolUse hook/Bash 审批;超时停任务保会话) · 新增 predict_skill MCP 补齐 compile→predict 诊断链 · lib.rs 人格/技能常量全退役改运行时物化。PM 2026-07-07 两轮指令定向;取代 ah-orchestration-design §9.8。**v4 复审修订(research §T6)**:spike 实测 SDK subagent 仅薄基座(基座+delta 前提四运行位差异化,手册须自足)· ah 1.4.0 对齐(cli.md 收缩纯 delta,`ah status` 条目作废)· R10 变更卡一键撤销+注入风险记账 · 指纹全资产覆盖 | [`docs/studio/mvp1/03_regions/copilot/ah-orchestration-design.md`](../../docs/studio/mvp1/03_regions/copilot/ah-orchestration-design.md) |
