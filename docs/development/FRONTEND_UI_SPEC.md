...
- **设计源里的「彩色 X」读作「看得出它不是普通文本」，不是「按分类配色」。**
  产品原话常写「彩色 pill」「彩色标签」，它要解决的是**这块东西和周围正文混在一起分不开**；
  把分类编进色相是另一件事，并且撞上上一条。判断顺序：先满足「与周围可区分」（一种
  底色/边框就够），再问「分类靠什么表达」——答案永远是文字与排版：分组标题、等宽、
  字重、图标形状，必要时 hover title 消歧。实例：copilot composer 的 @mention pill
  统一一种淡底，五个 kind 由菜单分组标题 + label 本身 + `kind · ref` 的 title 承载
  （2026-08-21，设计源 `copilot-assist` COPILOT_ASSIST-10 ⑤ 已据此订正措辞）。
...
- **虚拟化的行高不能靠猜:要么行高由内容决定,要么按真实高度测量。**
  固定行高窗口化(`index × 常量`)一旦和真实卡片高度对不上,就同时产生两个缺陷:短行被
  垫出大片空白,长行溢出到容器之外——而容器高度是按同一个常量算的,于是**列表尾部滚不到**。
  在它之上再叠一层 `space-y-*` 会让两种偏差同时发生(实测:容器 `n×128`,内容
  `n×128 + (n-1)×20`)。规则:①「行与行的间距」只允许有一个定义处;②数据量在几百条以内时
  直接自然流渲染全部行(容器高度天生等于内容高度),不要为了"看起来专业"引入虚拟化;
  ③真的需要虚拟化时,用按真实高度测量的实现,不要用常量估高;④键盘/选中滚动用
  `scrollIntoView({ block: 'nearest' })`,不要用 `index × 行高` 反推位置。
- **过滤器的控件数量不能由被过滤的数据决定。** 把原始枚举(事件类型、状态码、标签)逐个
  铺成 chip,控件高度就随数据增长,几十条记录就能吃掉半个面板。规则:把原始值归入**固定的
  少量语义桶**(桶必须**全覆盖且互斥**,新值有默认归属,并用测试锁住),桶常驻显示;确实必须
  逐个可选的维度(用户自己的节点名)用**单行横向滚动**承载,不换行。
- **面板里不放编辑器外观。** 用户不能编辑的内容不要用代码编辑器承载(行号、编辑器配色、
  编辑器滚动条):用面板自己的排版语言渲染(分组标题 + 卡片 + 等宽代码块)。同理,声称
  "完整"的视图不允许做**不可恢复的截断**——长内容可以默认收起,但必须能展开到全文。
...
- 文案要说**在等什么**，不要只写 "Loading"：`Checking…`（系统还没观测到状态）、
  `Starting…`（已经拉起、还没就绪）之类，各阶段一句。多个阶段同时成立时，**更具体的事实优先**。
...
### 2.7b 状态投影里，禁令压过证据（别让旧证据把当前事实顶掉）

- **一个对象的"现在不许用"和"它曾经能用"是两类事实，聚合成状态时不能平级投票。**
  凡是把一组子对象聚合成父对象状态的判据（"这个端点有没有可用路由"、"这个组里还有没有
  活着的项"），必须先问**当下有没有禁令**，再去看**历史证据**；禁令为真时直接出局，
  不给证据翻盘的机会。反过来做，就会出现"这一项明明标着停用、父级却是绿的"。
- **判据要区分"禁令"和"探测结论"。** 禁令是系统当下的决定（如 route 的
  `status: 'disabled'`——密钥被拒时后端整端点连同全部路由一起停用，下次测试成功再恢复），
  它是当前事实；而"上次探测失败"（`status: 'failed'`）是**结论**，可能只针对某个模型、
  可能是抖动，允许被其他证据（ready profile、`ui_state: 'ready'`）压过。两者不可混为一谈。
- **同一份事实在所有界面必须说同一句话。** 出现"路由 chip 是灰的、端点 chip 是绿的"这种
  自相矛盾，先怀疑聚合判据漏读了某个字段，而不是去改任何一个 chip 的配色。
- 反例（2026-08-12，Settings → API Keys 端点 chip）：Gemini 的 key 失效后,后端已按 R-E2
  停用端点和它全部 50 条路由、`last_test_message` 也写了 "Invalid API key",但
...
- API Key 输入框必须始终使用 `type="text"`，避免触发浏览器/扩展密码管理器。已有密钥的 ProviderCard 隐藏且未编辑时必须渲染与真实 key 等长的 mask 字符串，不把真实 key 作为 input 可见 value；redacted 态（值仍是 registry 的 10 位占位符、尚未 reveal）下「真实长度」来自 registry endpoint DTO 的 `api_key_length` 元数据，不许按占位符长度画点（2026-08-12 决议，否则用户把恒定 10 个点误读成 key 被截断）；输入框获得焦点或用户开始录入时进入编辑态，直接用真实草稿值作为受控 value 并明文显示，保证用户能看清正在输入和修改的内容，失焦后回到等长 mask 字符串；点 Eye 显示后也明文渲染真实 key。新建 provider 的 API Key 输入保持明文录入；空输入不得 mask placeholder。
...
### 2.9a 面板 chrome 预算（常驻控件必须自证价值）
- 原则：面板的价值在内容区，顶部/侧边 chrome 是从内容区借来的空间。**每个常驻元素都要能回答"用户拿它做什么"**，答不上来就删掉，答得上来但只是偶尔用就折叠成按需入口。
- 判据：一个控件如果在"当前没人在用它"的状态下仍然占高度，它就必须在该状态下**高度为 0**，或折叠进单个入口按钮。以 Trace 面板为例，常驻两行筛选 toggle 在 ~830px 面板里恒占 ~74px，而绝大多数会话根本不筛选——这就是不合格的常驻成本（决议 2026-08-08 D4）。
- 落法（本仓既有实现即范式）：
  - **搜索框独占整行**，不与任何按钮共享横向空间；挤到看不见 placeholder 属于缺陷。
  - **筛选走 `Popover` 按需展开**，已选条件在触发器旁以可就地删除的 chip 回显，无筛选时该区域不渲染。
  - **run / 文档级动作收进 `⋮` `DropdownMenu`**，它们属于对象身份而非内容列表；身份条上只留切换视图状态的小图标控件。
  - **handler 没接的控件不渲染**：只读视图里点了没反应的开关，等于一个恒假的按钮，必须省略而不是禁用（禁用见 §2.7a）。
- 列表同理：同一属性在连续多行里重复出现时，用**分组头**承载一次，不在每行重复打印（八条 `segment` 事件不该把节点名印八遍）。

### 2.9b 动作归属谁,就由谁渲染:别把选中态的动作放进条件挂载的组件里(2026-08-21)

- **原则**:一个动作属于**它的前提条件所在的那个东西**。属于「当前选中了什么」的动作,
  必须由**始终在场的那一层**渲染;属于「这次 run」的动作,才可以住在只有 run 时才挂载的组件里。
- **反面实证(台账 CP4)**:`Design golden` 的前提只有「选中了一个还没有 golden 的 agent 节点」,
  它却被放进 `TracePanel`。而 trace 区域在**一次 run 都没有**时挂的是运行历史列表,压根不挂
  `TracePanel`——于是这个按钮在**最需要它的那种 skill(全新、从没跑过)上根本不存在**。
  单测当时全绿,因为它们直接渲染 `TracePanel` 并传空事件数组:**测了组件,没测挂载条件**。
- **判据(动手前问一句)**:把这个动作的前提写成一句话,如果里面**没有出现**承载它的那个组件
  的存在条件,它就不该住在那个组件里。上例:「选中了一个没有 golden 的 agent 节点」里没有
  「有一次 run」,所以它不属于 `TracePanel`。
- **落法**:提成区域级组件(本仓范式:`components/trace/FocusedNodeActions.tsx`),由区域在
  各个主体之上渲染一次;每个动作各自保留**自己的**前提(创建 golden 只要节点,提升 golden 要有 run)。
  测试要打在**区域**上(`Panels.trace-mount.test.tsx`),而不是只打在组件上。
- **第二次发作,同一条规则,而且不限于「动作」(台账 L2 ③,2026-08-21 真机走查发现)**:
  节点 compare 的**候选页签**——一排用来在候选 side-run 之间切换的导航——同样住在
  `TracePanel` 的 live 分支里。同一批改动刚让「从历史 run 发起对比」跑得通,结果 side-run
  以 `success` 收尾而**一个页签都没有**:组存在,却不可达。把前提写成一句话是
  「存在一个 compare 组」,里面没有「有一次 live run」,判据当场成立。
  **所以本条约束的是「凡由选中/上下文决定其存在的界面元素」,不只是按钮**:导航、页签、
  状态条同样适用。落法照旧(`components/trace/CompareCandidateTabs.tsx` 由区域渲染)。
  **两次都是组件级单测全绿而功能不可达**——所以「测试要打在区域上」不是补充建议,是这条
  规则真正的门禁:组件级用例证明的是「给它数据它会画」,证明不了「它会不会被挂上去」。
...
### 2.11a 时刻一律换算成读者本机的墙钟(2026-08-20)

- **界面上任何一个人会读到的时刻,都由 `utils/wall-clock.ts` 换算成本机墙钟再显示;
  组件里不再自建时间格式化函数。** 后端与引擎存的一律是带时区的 UTC 瞬间(代码要拿它算,
  也要它在换时区后依然正确),而读者要的从来是自己那块表——这条不是前端偏好,是设计源
  `docs/studio/mvp1/02_capabilities/run-execution/mvp1-alignment.md` F1b 决策 D13
  的适用范围(原文:「UTC 戳对着文件树的人读起来就是错的时间」),那里是权威,本条只是指路。
- 四种读法各答一个问题,按问题选,不按格式选:`timeOfDay`(日期由上下文给定的行,如 trace 时间线、
  日志行)· `dateAndTime`(自己站得住的行,如运行历史、真相源最后写入)· `relativeTime`
  (「多久以前」比「哪一刻」更要紧时)· `fileStamp`(读数要当文件名活下去;与后端 run 目录同形)。
- **禁止在时间戳字符串的字面上做手脚**——切 `T`、抹掉尾部偏移量、`slice(0,19)`。同一瞬间可以
  写成 `Z`、`-07:00`、`+02:00` 三种样子,凡是动字符串的实现都会把它们渲染成三个不同的时刻,
  而其中至少两个是这次运行根本没有过的。实证:此前 Settings 真相源那行正是抹掉偏移量,
  于是把 UTC 数字当本地时间显示。门禁 `utils/wall-clock.test.ts` 直接钉这条不变量。
