...
# 14 — API Router（Studio LLM/Copilot HTTP 面）· MVP1 设计

> **Tier**：**③a Studio HTTP 适配壳，非 gateway 核心模块**。`routers/llm.py`/`routers/copilot.py` = Studio 把 HTTP 输入翻译成 service/gateway 调用的适配层（消费方），**不是 ③b gateway 公共内核**。判据：HTTP 端点形状、job/进度包装、DTO 投影绑死了"这个 app 的调用方式 + 存储介质"，换个 app 不会复用同一套 router → ③a 应用加工。
...
7. Copilot API 入口保持独立 router：HTTP/WebSocket 层转发给 `stream_query`，实际调用仍在 Copilot service（③a 领域，[[12-inv-copilot-invocation]]），见 `apps/studio/backend/app/routers/copilot.py:34-55`。
...
> router 是适配壳，测试关注点 = HTTP 契约 + delegate 正确性 + 不在 router 内重新实现内核语义。能力内核本身的测试归各内核模块（02/03/05/07/08/12）。
...
- **Copilot SDK test 走真 SDK**：`test_copilot_role_sdk` 改走 `ClaudeSDKClient`（非 `AsyncAnthropic`），否则 API 层给前端一个不可靠的"通过"信号（[[12-inv-copilot-invocation]]）。
...
- **待办**：把 Copilot SDK test 改为真实 `ClaudeSDKClient` 路径，否则 API 层仍会给前端一个不可靠的"通过"信号；归 [[12-inv-copilot-invocation]]。
...
| Copilot test | `test_copilot_role_sdk` (`:89-126`)（delegate `_probe_copilot_sdk_tool_call`，假测试，见 [[12-inv-copilot-invocation]]） |
