...
ground_truth:
  - packages/graph-agent（file:line 须复核;唯一真理源——mvp0 契约 skill-spec + workspace-spec 已迁 mvp1;机制区 _migration-src 尚未清完)
---
...
| `data-contracts` | **我们的**数据形状(BusinessData/FrameworkState/WorkflowState/result/ErrorPayload),**建在 langgraph 原语上**(原语是底座,不是我们的契约) | A 成段;B baseline 待成段 |
...
| `02-resolver` | 子图绝对 path 解析 + `SkillResolverProtocol` DI 接缝(边界/合法性校验;DI 不得隐式全局化) | ⏳ |
...
| `02-iterate` | 循环原语(batch/loop/图级);**图级 loop = B**(引擎包 loop-body) | A 摘要成段;B 仅节点级 batch |
| `03-checkpoint` | 共享 base(**建外层,经 `checkpoint_ns` 内外层共用同一个**)+ 外层 blackboard 存储/delta/有界 + durability | A 摘要成段;records 深度未迁完 |

**运行·内层(agent loop,LLM 驱动)** `05-run-inner/`
| `01-agent-loop` | `create_agent` 编排(model↔tool ReAct) | ⏳ |
| `02-middleware` | 6 槽链基础设施 + loop 卫生槽(ExecutionControl/LoopDetection);域专槽逻辑归各域(见 §6) | B:3 真 3 空;A 链基础成段 |
| `03-cognitive` | finish_task 处理(显式提交)+ 输出解析/patch(md2json) | A 主体成段;W 原话需补链 |
...
| `08-messages-state` | 内层 messages 持久化(DeltaChannel,经 `ns="<id>/agent"` 挂 checkpoint base)+ summarization + HITL/resume | B:Delta live,compaction/resume 未接;records 深度未迁完 |
...
| ACTION-TOOL | action(LOGIC,确定性,引擎调)vs tool(AGENT,LLM 调)= 两套;**不统一为 capability**(2026-06-04 已定) | 外/内主轴;纯 action(read-only dict)vs StructuredTool 本质不同 |
...
- **⏳ 成段化 / 未迁清**:`01-compile`/`02-resolver`/`03-assemble` 实现机制;`invalidation`(golden-stale 移 eval);`06-golden-eval` 按 golden→workspace 重写;`03-api-contract` 完整表;`11-io-and-edge-ops`、checkpoint/messages records 深度。详见 `INDEX.md` §3。

## 交叉引用(链接, 不复制)
README(章节去向)· 契约层 A(5)· 机制层 B(17)· API契约层 C(1)· mvp0(skill-spec + workspace-spec 契约已迁 mvp1,机制区不可整域 deprecated)· ../design/agent-loop-planA-create-agent-migration(迁移叙事)
