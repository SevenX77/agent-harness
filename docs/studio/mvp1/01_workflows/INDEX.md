...
### 走查状态
- ✅ **03 compile / 04 运行与验收 / 05 debug**: 全部走查 + **全量 atom actions 回写**进各节点文档(含决策 + 原话 + 测试关键点)。对引擎的设计需求见 `docs/engine/mvp1/`(契约层 + 机制层);早期 `_reorg/engine-prompt-*` / `gemini-prompt-batch-loop` 仅历史草稿、**非 SSOT**。
