...
- **workspace 落点(2026-07-05 import_files 收敛)**: 所有输入侧文件统一进入 `.workspace/import_files/`。Input 边界导入文件与 Test Inputs 新建 JSON 放在 `import_files/` 根目录;节点导入文件放在 `import_files/<node_id>/<import_name>/`。IO 面板显示已导入文件时只解析当前 scope 的 `source:'file'` 声明:Input/GRAPH 只看根级 `import_files/*`,phase 只看 `import_files/<当前节点 id>/*`;legacy 导入声明、旧测试输入目录或其他节点目录不得投影到当前面板。
