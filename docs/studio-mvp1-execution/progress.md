...
PM 经"最高裁判=设计文档/三模块>MVP1"裁定授权翻 api/llm.ts。原子前后端 flip:Studio 去 needs_setup→failed/missing_config、加 historical_ready,对齐 gateway canonical(`registry/projection.py`)+ FROZEN ux-spec。后端 gateway.py adapter + routers/llm.py(status_summary/admission/_force_probe_route metadata);前端 api/llm.ts 枚举+ModelGroupStatusSummary + 5 个 llm-roles 组件(badge/label/color/sort-rank/route-status,对齐 wave3 设计目标)。8-agent workflow + 独立 diff 复核(尤其 route.metadata reason_code 流)。门禁:后端 483、前端 tsc+vitest 423、e2e 7/2skip、lint/ruff clean。登记:dead `services/llm_state_projection.py`(薄委派 wrapper)留作单独 sink/删除任务。
