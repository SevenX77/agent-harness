...
仅在以下三种**才**停下问:① 真正无法自解的硬 blocker(工具链装不上 / 凭证**确实失效或根本不存在**——注意:真 key 在 `.env`/真库里就**不算** blocker,**直接用**);② 不可逆/破坏性动作且意图不明(注:在新分支上的可逆动作、**用真凭证只读验证** 都**不算**,直接做);③ 文档与代码都答不了的**业务**歧义(实现细节、存哪个字段、用哪个内部函数 = 自己定,不算)。**把手边可用的资源(真 key/真凭证/真 CLI)说成 blocker = 自造停下理由 = 违规(见 §1.4)。其余一律自主解决、不停。**
