...
| 页 | 看什么 | 状态点语义 |
|---|---|---|
| 节点主页 | 该节点的旅程/边界总览 | 节点级粗粒度实施状态 |
| 设计页（可多个 surface） | 每个操作原子「应该长啥样」+ 前后端契约 | 该 surface 全功能 **fe_status**（是否符合设计） |
| 后端接口契约 | 该节点全部后端机制/接口契约 | 全功能 **be_status**（后端那一半实现状态） |
| 前端复用模块 | 被多操作共用的前端组件登记 | 全节点 **fe_status**（前端模块实现状态，与契约页对称） |
| 实施 | 逐功能现状/差距 | 全节点 **fe_status**（前端实现状态） |
...
### 5.3 每页状态点怎么算（语义对称 + 测试看真机实测）
- 设计页 = 该 surface 全功能 `fe_status` rollup。
- 实施页 = 全节点 `fe_status` rollup。
- **后端接口契约页 = 全功能 `be_status` rollup**（后端那一半建好没）。
- **前端复用模块页 = 全节点 `fe_status` rollup**（前端模块建好没；与契约页对称）。
- **测试页 = 真机实测完整性**：`偏差`/`未实施` 测试=红；标 `shot_na`（无法真机实测）=琥珀；
  可真机实测的 `符合`=绿；混合→琥珀。**不许把测不了的涂成与真机证实同等的绿。**
  每个 `shot_na` 测试卡额外挂「⚠ 无真机实测」琥珀徽章，并在测试页提示框说明缺口。

> rollup 规则：全绿→绿、全红→红、混合→琥珀、无数据→不显示点。
