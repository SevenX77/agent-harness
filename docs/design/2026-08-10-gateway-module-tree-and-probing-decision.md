...
#### D7 的 P5 开工补记(2026-08-21:能力已落地,三处立项没写的取舍在此定死)

立项把 T3 写成"ToolCall(L1) → ReactLoop(L2)",落地时有三件立项没有规定、又必须有唯一
答案的事。它们定在这里,而不是留在代码注释里,理由是它们都属于"这次测量算不算数"的
判据,和 `EFFORT_CONTROL_LEVEL` 同级。

**一、对照值(control)是工具的返回值,不是提示词里的暗号。** 探测给路由一个工具、一个
只有调用工具才能回答的问题,工具返回一个模型不可能知道的随机码,终答必须**带着这个码
回来**才算闭环。没有它,"它读了工具结果"和"它编了一个像样的答案"落到屏幕上是同一句话
——这正是 `EFFORT_CONTROL_LEVEL` 为 effort 挡掉的那种混淆。工具描述里那句"这个码只存
在这里、推不出来也回忆不起来"是判据的一部分:少了它,散文式回答本身就是一个正确回答,
这次测量就什么也没测。实现:`probing/tool_loop.py` 的 `TOOL_LOOP_PROBE_TOOL` /
`probe_route_tool_loop(secret=...)`,调用方只在测试里指定 code,真探测自己现摇。

**二、只回喂一轮,轮次上限是 1。** 借的是 LangChain 自己的 `AgentExecutor`——它用
**迭代次数**给 ReAct 循环封顶,而不是指望它自己停;**拒绝的是它 15 次的默认值**,因为
那个数是按"把任务做完"配的,而探测的问题是二值的(这条路由出不出得来),每多一轮都是
用户真金白银的一次请求。到达上限时仍在叫工具的路由,记成**called_the_tool**(这是真的),
不记成闭环(这是没看见的)。

**三、这条探测永远不写 `tool_protocol=False`。** 和 `measured_image_input` 不同:那里的
拒绝是 provider 用话说"我只吃文本";这里"没看到工具调用"有两个分不开的成因——协议根本
没有工具,和模型就是选择用散文回答。写 `False` 会抹掉这条路由可能真有的能力,而**一个
悄悄缩水的 capability 比一个没测过的更坏**(与 `accepted_effort_levels` 返回 `None` 同一条
规则)。实现:`registry/capabilities.py::measured_tool_calling`,只有 True 一档;闭没闭环
写进 message,因为 capability 的值域是协议的是/否,装不下这个更强的观察。

**四个结果不是四个布尔,是一条梯子。** `ToolLoopReach` =
`no_answer` → `answered_without_calling` → `called_the_tool` → `closed_the_loop`,每一级
蕴含前一级。三个布尔能表达"没进过循环却出来了",一条梯子表达不了——让非法状态不可表示,
比事后校验便宜。`status` 和 `reach` 各自独立保留:前者说**为什么停**,后者说**走到哪**,
第二轮请求死掉的路由两样都要说得出来。

**尚未做完的是 P5 的后半段**:copilot CLI 收编为第三种执行器,以及"真机对至少两家 provider
各跑一次 L1/L2,结果写进路由能力"——后者要宿主先把这条能力接进 studio 的四个测试入口之一。
