...
| **前端 / UI** | 默认 **`agent` 自验**(2026-08-06 决议,取代 2026-07-06「详细点验归 PM」):agent 起 worktree 预览 + 冒烟 + 截图;合并收尾后在主 app 上**自己逐项点验**,交「逐项验证报告 + 截图」,PM 只看报告 | `scripts/wt-dev.sh`(动后端加 `--backend`),验自己端口,绝不验 5173;主 app 点验用 CDP 驱动真窗口(`RUN_AND_SCREENSHOT.md` §4)或 Playwright + sidecar bearer token | 否(PM 看报告,抽查可推翻) |
