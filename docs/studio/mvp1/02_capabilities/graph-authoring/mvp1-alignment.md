...
| ID | 决策 | 动机 |
|---|---|---|
| GRAPH_AUTHORING-1 | 子图字段 | 对齐 `subgraph-path-inline-drilldown` 设计单元，保证实现与测试可回扣 |
| GRAPH_AUTHORING-2 | inline 展开 | 对齐 `subgraph-path-inline-drilldown` 设计单元，保证实现与测试可回扣 |
| GRAPH_AUTHORING-3 | 节点态 | 对齐 `subgraph-path-inline-drilldown` 设计单元，保证实现与测试可回扣 |
