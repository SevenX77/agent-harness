...
- 约束(2026-07-02 R3,2026-07-05 修订):连接状态**不占独立一行**——连接正常(open)时什么都不显示;仅断连/重连时在标题旁显示紧凑提示 chip(含 retry 倒计时)。会话 tab 条 = 单行横向滚动(本地 ScrollArea,绝不出现系统滚动条控件),**每个 tab 带关闭按钮**:关闭 = 从当前窗口状态移除该 tab,不删除其落盘 transcript;关到最后一个时留一个全新空会话;关闭当前活跃 tab 时激活其前一个邻居。`+` 是动作菜单: `New chat` 创建并持久化空 session,`Restore chat` 打开原生文件选择器并默认指向本 skill 的 `.workspace/copilot/sessions/<skillId>` 目录。窗口状态由 `_window.json` 记录 `openSessionIds` + `activeSessionId`,打开 skill 只恢复该状态中的 tab,历史 session 文件只能通过 Restore chat 手动加回。
...
- 测试: closed websocket disables send; reconnect updates status; switching skills restores `_window.json` 中的 session window; closing a tab preserves transcript files; `+ → Restore chat` defaults to the current skill session directory.
...
## 6. 测试关键点
1. session UI: baseline 现状为 内存态 / skill 切换 reset 风险 ⚠️；目标为 顶部多 session tab 历史持久化,并用 `_window.json` 恢复上次打开的 tab 集合与活跃 tab。
...
## 8. gaps / 报警
- 🚨 session UI: 内存态 / skill 切换 reset 风险 ⚠️；目标 顶部多 session tab 历史持久化,并用 `_window.json` 恢复上次打开的 tab 集合与活跃 tab。
