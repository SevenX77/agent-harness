...
3. 上游决议的另一条遗留 —— `flow.working_memory` 是第二条跨相位通道 —— **本次不动**,
   仍待单独裁决。

## 附:`review` 相位的逐事件时序(把机制钉死的那份证据)

`review` 是全 run 唯一一个 15 次调用 / 3 次投递 / 6 次 nudge 的相位,乍看不合
"一次 nudge 后定格在 1"的模式。按 `trace.jsonl` 逐事件还原它的 4 次相位执行:

```
--- 执行 #1 ---  inject → llm_call → inject → llm_call → nudge → llm_call
--- 执行 #2 ---  inject → llm_call → nudge → llm_call → nudge → llm_call → llm_call → llm_call
--- 执行 #3 ---  llm_call → nudge
--- 执行 #4 ---  llm_call ×3 → nudge → llm_call → nudge → llm_call ×2
```

三件事同时被这份时序钉死:

1. **执行 #1 在第一次 nudge 之前投了两次。** 若投递是"只播种首轮",执行 #1
   永远不可能出现第二次 `inject`。这是"投递不持久化、每轮重投"最直接的一条正面证据。
2. **每一次 nudge 之后,同一次相位执行内再没有任何 inject**(#1 的第 3 次调用、
   #2 的后 4 次调用),因果方向清清楚楚。
3. **执行 #3 / #4 从第一次调用起就没有 inject** —— 它们开场就继承了别的相位的
   HumanMessage,属于 #838 修掉的跨相位那一半。

3 次投递 = 执行 #1 的 2 次 + 执行 #2 的 1 次,15 次调用、6 次 nudge 全部对上,无残余。
