...
Claude code 或 Codex 跑;Windows 上 `ah` + provider CLI 活在 WSL2 里。Claude 登录源
是 Windows `%USERPROFILE%\.claude\.credentials.json`(WSL/ah sandbox 只 symlink,不复制);
Codex 登录源是 Windows `%USERPROFILE%\.codex\auth.json`。**装 + 用的完整指南**(含一次性安装、
点菜单的使用流程、排错、所有权边界)见:
