...
| Copilot context | Workspace no longer sends selected node/edge/lint status into Copilot context; UI selection remains local until a future composer `@` mention explicitly sends it. | `apps/studio/frontend/src/components/studio/Workspace.tsx:Workspace`, `docs/development/STUDIO_REQUEST_AUDIT.md` |
