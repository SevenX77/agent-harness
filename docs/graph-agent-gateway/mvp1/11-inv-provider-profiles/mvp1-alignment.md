...
> **决策日志**：`.kiro/specs/studio-llm-gateway-redesign/client-layer-decision-record.md`（F6 provider 差异 → init-kwargs profile / M6 `RouteChatModelFactory` / D1 方案 A' / §4 借鉴 vs 自建）+ `docs/graph-agent-gateway/mvp1/module-disposition-revised.md`（§4 判 11 纯 ③b 新建）
...
5. `RouteChatModelFactory` 调用 `ProviderProfile`。`RouteChatModelFactory`(route 到 ChatX 的构造器,归 [[10-inv-route-chat-model-factory]])应先从 `ResolvedRoute` 生成基础 kwargs，再叠加 provider/profile defaults，最后叠加 route runtime settings，形成 `ChatAnthropic`/`ChatOpenAI`/`ChatGoogleGenerativeAI` 的 init kwargs(`packages/graph-agent-gateway/src/graph_agent_gateway/registry/schema.py:415-445`; `.kiro/specs/studio-llm-gateway-redesign/client-layer-decision-record.md:157-163`)。
6. provider 差异优先写进 init kwargs 表。headers、Responses API、base_url 参数名、温度默认、stream_usage、thinking 开关等构造期差异应通过 profile 解决，而不是复制 `_call_*` 的消息转换(`.kiro/specs/studio-llm-gateway-redesign/client-layer-decision-record.md:224-228`; `temp/deerflow/backend/packages/harness/deerflow/models/factory.py:34-47`; `temp/deerflow/backend/packages/harness/deerflow/models/factory.py:94-146`)。
...
| **payload patch 边界** | 仅当请求 payload 必须改，才子类覆盖**单方法**（如 `PatchedChatDeepSeek` 只覆盖 `_get_request_payload`）；**绝不重写整套消息转换**（A' 核心，`client-layer-decision-record.md:224-228`）。 |
...
> **F6 provider 差异 → init-kwargs profile**（决策记录 `:242-246`，含 PM 拍板的方案）："用 deepagents `ProviderProfile` 模式（provider/model → 一张 init-kwargs 表 + 可选 `pre_init`/factory）装 provider 差异（headers、responses api、温度默认、thinking 开关等）；仅当需改请求 payload 才子类覆盖单方法（deerflow 范式），绝不重写整套消息转换。" → 本模块即 F6 的落点。

> **D1 方案 A'（不重写整套消息转换）**（决策记录 `:41`）："不用留A, 这是错误判断, 正确的是A'。" → A' 用原生 ChatX 接管消息转换/调用/解析；provider profile 只能影响构造参数，不能回到自研 dict payload 大锅。
...
1. 不重写整套消息转换。A' 的核心是用原生 ChatX 接管消息转换、调用、解析；provider profile 只能影响构造参数，不能回到自研 dict payload 大锅(`.kiro/specs/studio-llm-gateway-redesign/client-layer-decision-record.md:82-97`; `.kiro/specs/studio-llm-gateway-redesign/client-layer-decision-record.md:224-228`)。
...
> 来源 = 决策记录 §5「兼容性验证清单（A' 实现必过）」`:266-275`。本模块（provider 构造差异表）对应其中与 init-kwargs 合并 / thinking / stream_usage / payload patch / 边界不混相关的项。
...
- **payload patch 仅单方法**：若移植 `PatchedChatDeepSeek`，仅覆盖 `_get_request_payload` 恢复多轮 reasoning_content，**不重写整套消息转换**（A' 边界，决策记录 `:224-228`）；移植时一并核对 deerflow 依赖的 `assistant_payload_replay` helper（§8 待办 3）。
...
- **capability 不动态选型**：profile 表不引入按 capability/price/latency 搜索替代 route 的逻辑；capability 仍只 lint/warn/block（MVP0 alignment `:142-146`）。
- **异常分类形状不回归**（关联项）：用 profile 构造的 ChatX，其 retry 耗尽异常仍能被 07 `classify_exception` 正确分类（决策记录头号风险 `:268`）。
...
- 决策记录 `client-layer-decision-record.md` F6/M6/D1/§4 / 归属表 `module-disposition-revised.md`（§4 判 11 纯 ③b 新建）/ `mvp0-alignment.md:142-146`（capability 不驱动选型）
