...
> 下面三条是这套架构的**逻辑底**,不含任何代号;后面 §3 的技术命名只是它们的实现映射。三条都指向同一个目标——**逻辑上模块化、解耦、高内聚**,不为实现便捷牺牲它。

**底座一:单一真相源,会变的真相不缓存。**
每一类数据只有**一个真相源**,谁拥有它、谁负责读写。会变的真相(凭证、设置、角色)**绝不允许旁路缓存**——缓存让真相分叉是踩过的 bug。凭证的真相在 gateway(本地模拟远端 = 借 studio 后端专门接口写本地文件 / 真远端 = 写库),写入只走"FastAPI → gateway → 唯一真相",前端只投影、不持第二份。

**底座二:三种性质不同的数据,按性质分治,不塞一个万能接口。**
系统里有三种本质不同的数据——① **配置真相**(小、要强一致、读多写少)② **运行产物**(大、一次写成、写完不改)③ **运行中的活状态**(临时、要快、被单个运行实例独占)。三者的读写方式 / 一致性 / 生命周期都不同,各自是一个**内聚、互不耦合**的存储职责;硬塞进一个"通用存储接口"= 四不像 = 图实现便捷,正是要避免的。

**底座三:创作与运行彻底解耦,运行端只认冻结成品。**
"创作 / 编译 skill"和"运行 skill"是两件本质不同的事。创作端把源码编译成一个**冻结的成品**并给它**版本身份**;运行端**只认冻结成品 + 版本身份**,按版本取来跑,**不碰源码、不需要同步源码**。这样创作和运行干净解耦,在跑的版本不会在脚下被改,"源码同步"这个问题直接消失。

> **映射**:底座二 → §3 的三套存储插座(配置 / 产物 / 运行态);底座三 → §3 的发布流水线 + 跨宿主交接;底座一 → §3 各线的真相归属 + 缓存边界。
