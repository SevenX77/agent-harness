...
| # | 模块问题 | 层 | 状态 | 出处 | 验收判据 |
|---|---|---|---|---|---|
| R1 | run 报告漏答 08-08 规格里的五个问题 | 后端(run 报告投影) | 🧩 **11 项核过:6 项本就齐,5 项缺。本轮修掉 3 项(②④⑤),剩 2 项(①③)另一 PR。** 齐:整体情况 / 花费时间 / token / 每节点时间-token-模型 / 文件链接 / artifacts 链接。**本轮已修**:②**batch/loop 详情**——新增 Repeats 节:`iterate` 的每次执行各自一行(耗时/token/工具/成败),`parallel_map_group_started/ended`(引擎一直在发、报告一个都没读)按组给出 item 数/并发度/成败/墙钟;同时把误标的列名 `loop iterations` 改成 `agent turns`——它数的是**一次执行内部**模型想了几轮,不是节点跑了几次。④**报错详情**——Failure 现在收 `loop_detected`(转圈被切)与 `builtin_subagent_fallback`(**跑完了但走的是更次的路**,不报就没人报),单条消息截到 200 字符(实测一条几千字会把同节其他失败全压出屏幕,全文在报告已链接的 `trace.jsonl` 里);`nudge`/`tool_error_handled`/`tool_history_repaired` 归为**纠正**只计数(Nodes 表 `corrections` 列),不进 Failure。⑤**Nodes 表状态列**——`ok`/`failed`/`interrupted`/`unfinished`(开了没关,既非成功也非失败),节点状态取各次执行里最差的。设计源同 PR 落盘:RUN_EXECUTION-8/9(哈希已重钉)。**仍缺**:①**input files 是 code 文本不是链接**(`run_report.py` `_inputs_section`),数据源是 `runtime_config.snapshot` 的声明绑定,未叠加引擎实发的 `input_file_injected`;③**llm vs 结果零链接**——只打印 group/candidate id,且**基准 run 的报告里根本没有 Model compare 节**。另有一条结构缺陷未修:报告只在 run 终态写一次,**没有重生成入口**,而 RUN_EXECUTION-5 明说它是可随时重生的纯投影——历史报告永远停在写它那天的渲染逻辑上 | 08-08 大单 | 逐项在报告中找到 |
