...
| P1d | probing/ 域 | ⏳ 并入 P2c | 只剩 `registry/provider_probe.py` + `probe_contracts.py` 两个模块待搬;它们的归位与 dialect 抽取是同一件事,分开做等于搬两次 |
...
| P2c-2 | 探测把「哪一家」和「怎么说话」拆成两个问题 | 🚧 待合并 | 原来 `endpoint_probe_backend` 一个答案身兼二职,于是主机名能推翻用户声明的协议。现在 `probe_wire_backend(protocol)` 决定请求长什么样(依据:**生产就按协议分派**,`dispatch.py:189`),`endpoint_probe_backend` 只回答厂商身份(studio 的官方方法菜单按它取 `probe_candidates.json`),且只认 `deepseek.com` 域 + DeepSeek 真发布过的那两个协议面,**不再看 endpoint 的名字**。100 条基线 37 条变化,三类:协议不再被主机名推翻、名字不再决定菜单与字段、OpenAI 兼容面预算字段改 `max_tokens`(`_call_openai_compatible`(dispatch.py:239)对所有 openai 兼容路由都发这个;探测原本替 OpenAI 造了个生产从不发的 `max_completion_tokens`、却替 DeepSeek 发对了)——改齐后 **DeepSeek 的 openai 兼容 endpoint 一条都没变**。**中途踩过一次**:先做的版本把两问题合并成「一律按协议」,studio 全量才炸出官方菜单被悄悄删掉;教训是删概念前先问它在替谁回答问题。同期拆开 `provider_probe_backend_for_method` 混在一起的两个报错 |
| P2c-3 | 抽 `judge` + `probing/` 域,`_AWAITING_REHOME` 清空 | ⏳ 待办 | `_probe_status` 与几个响应读取函数(`_model_ids`/`_model_capabilities`/`_provider_response_message`/`_extract_vendor_error_*`)合成判据单元;`provider_probe.py`/`probe_contracts.py` 搬进 `probing/`。纯搬迁,两份基线一行不动 |
