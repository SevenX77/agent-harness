...
> **Tier**：③b gateway 公共能力内核（`capabilities`/`lint`/`profile_selector` 已在包内；**model_groups / identity / notable / route_capabilities 的能力内核也属 ③b，现散 ③a `apps/studio/backend/services` 待下沉**）
...
> **决策日志**：`packages/graph-agent-gateway/README.md` §2/§3 + `docs/studio/mvp1/01_workflows/00_settings-ux-spec.md` §6.0 + `docs/graph-agent-gateway/mvp1/module-disposition-revised.md`（行 31-35 四项反转）
> **现状**：见同目录 `baseline.md`
...
**上下游**：① provider raw metadata / list-models / probe result →〔**归一化 ③b**〕`normalize_route_capabilities` → route capabilities → 〔**合并 ③b（现散 ③a）**〕`route_effective_capabilities`（并入 ready verified profile facts）→ 〔**描述符 ③b**〕`build_runtime_setting_descriptors` → 前端控件；同一份 route capabilities 也被 〔**分组/识别/知识 ③b（现散 ③a）**〕`project_model_group_identity` / `project_model_identity` / `notable_model_ids` 投影成 Available Models 模型知识 DTO（③a 套上 family 折叠 / 弃用区 / 展示名样式）。② role 解析时：`resolve_role` 按 `fallback_chain` 取 route → 〔**profile 选择 ③b**〕`select_verified_profile` 选单 route 内调用方式 → 〔**lint ③b**〕`lint_role_routes` warn/block。

**关键边界（判据，目标语义）**：capability = route **内在事实**（支持/默认/边界），不承载用户本次请求意图；runtime intent = role/profile route entry 的 `RuntimeSettings`；profile selection = 单 route 内"怎么调"，不跨 route 选"调谁"；lint = 配置守门，不按能力补 route。**model group / identity / notable / route_capabilities = 模型知识的客观加工**（分组/归类/合并都不依赖 UI/产品策略/调用方式/存储介质），换任何 app 都要 → **③b 公共**。

**目标设计与流程**（逐步，保留原"目标设计与流程"全部 + 判据标注）：

### 2.1 编排目标（原 §1「编排目标」全保留 + 判据校准）

1. capability 是 route metadata。`CapabilityValue`（保存能力值、来源、观测时间、说明文字的结构）用于保存能力值和来源，**不得承载用户本次请求意图**（`registry/schema.py:67-75`，`docs/graph-agent-gateway/mvp0/provider-runtime-settings-matrix.md:5`）。**判据**：归一化是 ③b 公共（README §3.C）。
2. runtime intent 是 role/profile route entry。`RuntimeSettings`（保存用户写在 route entry 上的 provider-neutral 运行参数的结构）用于保存 temperature、max_output_tokens、reasoning 等（`registry/schema.py:121-135`，`docs/graph-agent-gateway/mvp0/provider-runtime-settings-matrix.md:32-42`）。
3. profile selection 是单 route 内的"调用方法选择"。`select_verified_profile`（在当前 route 的 ready profiles 中选 method_id/request_mapper_id 的函数）用于在当前 route 选调用方式，**不跨 route 找替代模型**（`registry/profile_selector.py:14-52`）。**判据**：单 route 内选择 = ③b 公共。
4. lint 是配置守门。`lint_role_routes`（对显式 role fallback chain 逐 route 检查能力要求和 runtime 边界的函数）用于产出 warn/error/blocking，blocking 可让解析失败，但 **lint 不改变 fallback_chain，也不根据 capability 动态补 route**（`registry/lint.py:27-87`，`registry/resolver.py:116-122`）。
5. **模型知识投影是 ③b 公共能力内核（本轮反转，现散 ③a 待下沉）**。`project_model_identity`（把 route/endpoint 名解析成品牌/家族归类的函数）、`project_model_group_identity`（把同模型多变体折叠成一张模型组卡身份的函数）、`notable_model_ids`（从 provider notes 文档提取已知可用 model id 的函数）、`route_effective_capabilities`（把静态声明能力 + ready verified profile 派生能力合并的函数）= **把原始模型数据标准化/组织/合并/沉淀知识，换任何 app 都要 → ③b 公共**；它们**不改变 runtime route_id**（runtime 仍按显式 `route_id` 执行）（`apps/studio/backend/app/services/llm_model_identity.py:83-121`、`apps/studio/backend/app/services/llm_model_groups.py:43-68`、`apps/studio/backend/app/services/llm_notable_models.py:16-37`、`apps/studio/backend/app/services/llm_route_capabilities.py:10-64`）。**留 ③a 的只有**：展示名样式覆盖 / family 折叠 / 弃用区分区 / notable 在哪个面板展示 + 数据源路径注入 / identity 展示投影渲染。**被否**：原 §1 #5 把它们整体判成"显示/解释层、不改变 runtime route_id、属产品解释"——其能力内核被错划 ③a，现反转为 ③b 公共。

### 2.2 编号执行流程（原 §2「编号执行流程」全保留 + 判据标注）

1. 导入/探测阶段拿到 provider raw metadata 或 probe result，调用 `normalize_route_capabilities`（把 provider/API/raw metadata 归一成 route capability 字典的函数）归一化为 route capabilities（`registry/capabilities.py:35-202`，`apps/studio/backend/app/routers/llm.py:806-815`）。**判据 ③b**。
2. Studio registry API 调 `build_runtime_setting_descriptors`（把 route capability 转成前端可安全渲染的 runtime setting 控件描述的函数），把每条 route 的 capability 投影成固定 11 个 runtime setting 控件描述（`registry/capabilities.py:205-217`，`apps/studio/backend/app/routers/llm.py:1377-1380`）。**判据 ③b**（控件描述符是公共能力，前端只选"关心哪几种能力" → README §3.C）。
...
> 跨边界签名 / schema / 错误 / 归属，单独成段。`ResolvedRoute`/`ResolvedRole`/`CapabilityValue`/`VerifiedProfile`/`LintResult` 字段权威源 = [[04-orch-registry-schema]]（`registry/schema.py`），本模块**只链接不复制**，防 drift。
...
| **控件描述符 → ③b** | `build_runtime_setting_descriptors(capabilities) -> list[RuntimeSettingDescriptor]`（`registry/capabilities.py:205-217`，`RuntimeSettingDescriptor` schema `registry/schema.py:147-160`）。固定输出 11 个控件描述（key/类型/支持状态/上下限/默认/枚举/来源），驱动 ① intent 控件；前端只选"关心哪几种能力"，不硬编码 provider 规则。 |
...
| **capability_state 第二轴（D2，待补四态）** | `_capability_state(route) -> Literal["unknown","callable_only","partial","known"]`（现散 ③a `routers/llm.py:1767-1770`，现二值待升四态）。**判据 = 按每个 capability 的 `source` 算（probe-verified vs doc/list/draft 声称 vs 未知），不并入 `route.status`（availability）**。投影侧落点见 [[08-orch-test-status-ssot]] 回填。归属 = route 内在轴（③b 标准总结，现散 ③a）。 |
...
> **判据（通用，每模块引）· README §2 行 44 + ux-spec §6.0 行 334（verbatim，不改一字）**：
...
> **本轮反转的 PM 校准 · ux-spec §6.0 行 334（verbatim）**：
...
1. **模型知识能力内核 = ③b 公共（本轮反转，原 §决策原因 #5 被否）**：把原始 model id 客观**分组（model_groups）/ 归类品牌家族（identity）/ 沉淀已知可用知识（notable）/ 合并静态+探测能力（route_capabilities）**，是 gateway 对模型数据的标准化/组织/知识沉淀，换任何调模型 app 都原样要用 → **③b 公共**（README §3.B、disposition 表行 32-35）。**被否**：原 §决策原因 #5 判"Studio model identity 和 model group 留在后端 service，原因是展示归一化属于**产品解释，不是 Gateway runtime schema**；代码注释要求 routers 调 service 不在路由内联清洗"——其中"调 service 不内联"作为代码组织约束仍对，但"属产品解释/不是 Gateway schema → 留 ③a"的**定调全部反转**为"③b 公共能力内核，现散 ③a `apps/studio/backend/services` 待下沉"。**留 ③a 的只有**：展示名样式覆盖、family 折叠、弃用区、notable 数据源路径注入、identity 展示投影渲染。
2. **选择显式 route chain 而不是动态能力搜索**（原 #1 保留）：让 role 执行可复现、可解释、可由 Studio 审核；README 明确禁止按 provider/capability/price/latency/availability 搜索替代 route（`docs/graph-agent-gateway/mvp0/mvp0-alignment.md:142`）。**注**：下沉 model_groups/identity/notable/route_capabilities 到 ③b **≠ 引入动态选型**——它们是**分类/知识/能力描述**，runtime 仍按显式 `route_id` 执行（README §3.E lint「不替应用选型」、disposition §0）。
...
- **lint 只产结果不改链**：blocking lint → `RegistryResolutionError`，但 `fallback_chain` 顺序/成员**不变**；非 blocking → 进 `ResolvedRole.lint_results`（防 lint 退化成动态选型）。
- **模型知识 = 客观分组、不影响执行**：`project_model_group_identity` 折叠同模型多变体后，runtime **仍用精确 `route_id`** 执行（`llm_model_groups.py:48-53` 注释「exact execution still uses each route_id」必测）。
- **capability_state 四态（D2 回填）**：`unknown`(无线索)/`callable_only`(key/endpoint 通无契约验证)/`partial`(部分契约 probe-verified)/`known`(全 verified)，**按 per-capability `source` 算，不并入 `route.status`**；`tools`/`structured_output` 改派生（不再硬编码 `unknown`，`llm.py:1778-1779`）。
- **无动态选型回归**：05 不新增任何"按 capability/price/latency 自动找别的 route"路径（架构边界，非缺功能）。

## 7. 涉及 region / platform

- **③b** `packages/graph-agent-gateway`：`registry/capabilities.py`（归一化 + 描述符）、`registry/profile_selector.py`（profile 选择）、`registry/lint.py`（lint）、`registry/resolver.py`（调用上述三者的使用链）—— 均已在包内。
- **③b 公共能力内核（现散 ③a 待下沉）** `apps/studio/backend/app/services`：`llm_model_groups.py`（分组）、`llm_model_identity.py`（品牌/家族识别）、`llm_notable_models.py`（已知可用知识）、`llm_route_capabilities.py`（能力合并）；`_capability_state`（现 `routers/llm.py:1767-1770`，待升四态）。
- **③a 应用加工（留 studio / 前端）**：展示名样式覆盖、family 折叠、弃用区分区、notable 在哪个面板展示 + 数据源路径注入、identity 展示投影渲染、6 态/capability_state 颜色转 DTO。
- **② Rust**：N/A（角色/凭证/能力数据永不 Rust）。
...
覆盖率：8/8 个指定文件 100% 映射；公共入口 13/13 已纳入流程；关键调用点 `resolve_role`、`probe_route`、`_registry_response` 也已索引。本文未覆盖 [[11-inv-provider-profiles]] 的新增 provider profile 适配实现（README 放在调用层模块 11，`docs/graph-agent-gateway/mvp1/README.md:40`）。
...
- `docs/graph-agent-gateway/mvp1/README.md:30` — 模块覆盖 brief。
- `packages/graph-agent-gateway/README.md:59-69` — §3.B 模型知识 + §3.C 能力（model group/identity/notable/route_capabilities 标 🔻 = 公共但现散 ③a）。
- `docs/graph-agent-gateway/mvp1/module-disposition-revised.md:31-35` — 四项反转判定（model_groups/identity/notable/route_capabilities → ③b 公共）。
...
## MVP1 回填 — capability 就绪轴（D2，PM 2026-06-03 需求对账补；接上方 gaps）

> 源：`studio-llm-platform-control-plane-runtime` R5.3「role 要 thinking/tools/structured/vision 时，capability 就绪与 availability 就绪分开记」；studio `00_settings-ux-spec.md §4.2` 已立 `capability_state` 轴。
...
- **轴归属（Claude 定，PM 可推翻）**：`capability_state` 是 **route 内在轴**（ux-spec §4.2「了解多少能力」），按 route **声称**的 capability 集算；"适不适合本角色"是另一条 `role_fit`(role 级)——两者不混。判据归属 = ③b 标准总结（现散 ③a 待下沉）。
