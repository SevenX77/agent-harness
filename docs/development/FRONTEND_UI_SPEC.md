...
- **子图展开不得重置视口**: 点击 subgraph 节点右侧 `+/-` 只切换 inline preview 展开态，不得触发 `fitView`、auto-center 或自动移动 canvas 焦点；只有节点坐标或边拓扑真正变化时才允许自动 refit。
...
- **画布内"内联/预览"另一张图，必须复用同一个 skill graph renderer，不许临创局部 builder**: 子图 inline 展开这类"在画布里嵌一张真子图"的需求，子图节点/边一律走主画布同一条图构建管线——`buildNodesFromTopology`（出子图自己的 `__global_input__`/`__global_output__` 全局节点 + 真 phase 节点）+ `buildEdges`（`contextEdge` 连接边，自带可点中点圆点）+ `getAutoLayoutedElements`（dagre TB）——然后交给 `SkillGraphPreview` 用同一套 `SkillNode` / `GlobalInputOutputNode` / `ContextEdge` / `ReactFlow` 渲染。外层 `subgraph-expansion.ts` 只负责放置 `SubgraphGroupNode` 和 parent↔group bridge；不要再手搓子图 overlay 节点/边。桥接父子用单独的虚线 bridge 边接驳 `expand 节点 ↔ 子图预览 group`，不接到具体 phase 上。
...
- Expanded subgraphs must not be rendered with a nested `ReactFlow` or `SkillGraphPreview` inside a node. Emit the child topology into the parent canvas as read-only, id-prefixed sibling nodes/edges using the normal `SkillNode` / `GlobalInputOutputNode` / `ContextEdge` path.
- The parent-child bridge starts from the parent subgraph `+` handle and targets the expanded child graph Input's left handle. The child graph is positioned from that entry point, not centered against the whole parent graph.
