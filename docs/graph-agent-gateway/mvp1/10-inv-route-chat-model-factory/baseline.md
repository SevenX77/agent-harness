...
6. `openai_compatible` 和 `ark_runtime` 构造 `ChatOpenAI`；DeepSeek route 使用 `PatchedChatDeepSeek`，仅在 `_get_request_payload` replay 多轮 assistant `reasoning_content`；Ark base_url 归一到 `/api/v3`。
...
9. factory 通过 `route_provider_profile_keys(route)` 派生 `protocol:{protocol}`、`endpoint:{endpoint_id}`、`endpoint:{endpoint_id}:model:{provider_model_id}` 三层 key，再调用 `apply_provider_profile_layers(..., route=route, **kwargs)`，让 caller kwargs 最终优先。
10. 未识别 protocol 返回 `GenericRouteChatModel`；factory 会把 `max_tokens/temperature/top_p/stop/seed/reasoning_effort` 等调用期 kwargs 传入 generic adapter。
...
- `PatchedChatDeepSeek`:已存在，仅覆盖 `_get_request_payload`，把多轮 assistant `reasoning_content` replay 到 DeepSeek provider payload。
- `GenericRouteChatModel`:已存在，最小 ordinary-chat adapter 已实现，可经 `create_agent` 跑通工具循环。
- `GatewayChatModel._dispatch`:ChatX 主路径入口。
- `canonicalize_base_url`:factory 调用侧 base_url 幂等归一共享原语。
- `ordinary_chat.dispatch_ordinary_chat/_dispatch_provider_call/_call_*`:generic ordinary-chat provider core；官方 ChatX 主路径不直接调用，`GenericRouteChatModel` 默认 dispatcher 使用 `dispatch_ordinary_chat`。
