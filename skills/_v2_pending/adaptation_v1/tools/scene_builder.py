...
def build_objective_scenes(ctx: dict) -> str:
    ...
    for ev in events:
        ...
        if (
            current_scene
            and current_scene["meta"]["location"] == loc
            and current_scene["meta"]["time"] == time_period
        ):
            ...
            current_scene = {
                ...
                "meta": {"location": loc, "time": time_period},
                "source_events": [ev],
    ...
    summary = (
        f"成功生成 {len(objective_scenes)} 个客观物理场。"
        "请将以下内容分别放入 task_tool 中并发执行提取：\n\n"
    )
    for sc in objective_scenes:
