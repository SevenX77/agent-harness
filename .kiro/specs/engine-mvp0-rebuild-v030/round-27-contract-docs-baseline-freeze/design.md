...
## §0 审计驱动的 Scope 修正
最初的契约设计仅将 `graph_agent.__all__` 导出的 18 个符号作为防漂移防线。在随后的 `CONSUMER-API-INVENTORY` 审计中发现，下游宿主（特别是 Studio Backend 与 Predict V2）实际上强依赖了 **65 个符号**，其中包含 12 个 `_predict_internal` 私有模块的符号以及大量 Callback Events、AST 等深层对象。二次审计发现 `packages/` 漏扫，gateway 包补 4 符号，且 `_predict_internal` 实际 12 非 11，故权威 scope 从 61 修正为 65。
**设计修正判据：“被消费的即是契约 (What is consumed is the contract)”**。如果我们只冻结 18 个符号，在未来的重构中修改了其余 47 个符号会导致测试放行但生产环境崩溃，这直接违背了 PM 的 Golden Principle。因此，本次冻结设计全面转向**“真实 65 符号表面”**，并引入更强硬的哈希锁、字段级断言和测试绑定矩阵机制。

## 1. 契约边界设计与 `_predict_internal` 处理方案

### 1.1 真实的契约 Scope：冻结 65 表面
- **定义**：API 漂移防御必须覆盖完整的 65 个已被消费的符号。
...
- **Vendor-only 特殊标记**：其中有 6 个符号（`AgentSkillDef` / `GraphSkillDef` / `IoInput` / `PersonaSkillDef` / `CompileIssue` / `parse_skill_file`）仅被 Tauri vendor 副本消费，真实 Live 并不依赖。这 6 个符号依然冻结进 65（宁多勿少保证安全），但在 API 文档中需明确标注为 `vendor-only / 待核实`，为 PR 2 的边界清理留存上下文信息。
...
- **残余风险说明 (Matrix-Gaming)**：接受自动化验证存在的“套利风险”（即一个功能绑定了一个存在且能 PASS 但实际上并不测该功能的 Test，俗称 gaming）。自动化难以彻底防止此行为，因此明确其为 accepted Medium residual risk，需依赖后续 a3 Audit 人工核对真覆盖率，不作为 CI 自动化阻断项。

### 2.2 公开 API 契约与字段级漂移防御 (Field-level Drift Prevention)
- **强制机制**：只检查符号是否存在是不够的，如果 Dataclass 删除了一个字段，同样是致命漂移。
- **落地**：防漂移测试 `test_public_api_contract.py` 必须使用 AST 或 `inspect` 锁定完整的 65 符号表面，并且对所有的对象执行**签名与字段级断言**。对于 Dataclass 或 Pydantic Model（如各种 Event 和 AST），需提取其属性名和类型进行硬编码断言（如确保 `PathDiff` 依然包含 `added`, `removed`）。

### 2.3 文件标准冻结 (skill-spec Hash Lock)
- **强制机制**：为了防止在不经意间绕过 `status: FROZEN` 修改正文（内容守门）。
- **落地**：防漂移测试中必须计算这 14 份文档的内容哈希（SHA-256 Snapshot）。执行细节上，**SHA-256 计算须采用二进制读取（`rb`）或统一标准化行尾**，以避免因跨平台 CRLF/LF 换行符带来的假阳性漂移错误。任何哪怕多了一个空格的修改，都会导致哈希变动从而 CI 失败。这彻底剥夺了“悄悄改字眼”的可能性。
...
- **授权绑定保护**：为防止任何人随意修改 YAML 自我放行，`contract-exemptions.yaml` 文件本身**必须受到 CODEOWNERS 或 GitHub Branch-Protection 保护**，强制要求 PM 或指定负责人的 Approve (人工 Review) 才能合入修改。无物理保护授权，“批准门”仅沦为留痕而无实际防御意义。
...
- **PR 1: 本次 PR (The Gatekeeper PR)**
  - 交付：完整的 65 符号 API 文档、带测试绑定的功能清单矩阵、skill-spec 文档打上 FROZEN。
