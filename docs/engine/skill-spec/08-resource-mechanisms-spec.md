...
Example 是“案例”, 不等同于 Reference。V0.3.0 只支持两种模式:

| 模式 | frontmatter | 装配期行为 | runtime 行为 | 适用场景 |
|---|---|---|---|---|
| inline | `type: inline` + `content` | 直接 splat 到 `<examples>` 的 `【内联示范】` | 不需要 tool | 短小、稳定、强相关的判断边界 |
| document | `type: document` + `path` + `summary` | 只把 id + summary 列入 `【扩展案例库】`, 不预读原文 | Agent 可调用 `read_example(example_id)` | 长案例、低频边界、会挤占 prompt 的材料 |

字段级规则:

| 字段 | inline | document | 校验失败错误码 | 业务作用 |
|---|---|---|---|---|
| `id` | 必填 | 必填 | `[F-v3-resource-example-invalid]` | `@example:E1` 和 `read_example` 的 key |
| `type` | 必须 `"inline"` | 必须 `"document"` | `[F-v3-resource-example-type-invalid]` | 决定装配策略 |
| `content` | 必填 | 禁止 | `[F-v3-resource-example-invalid]` | 直接注入 prompt 的短示例 |
| `path` | 禁止 | 必填 | `[F-v3-resource-example-path-missing]` | runtime 按需读取的文档路径 |
| `summary` | 可选 | 必填 | `[F-v3-resource-example-summary-missing]` | 给 LLM 决定是否读取的目录说明 |
...
| `type` | enum | 是 | 无 | 只能是 `inline` 或 `document` | `[F-v3-resource-example-type-invalid]` | 决定装配或按需读取 |
| `content` | string | inline 必填 | 无 | `type:inline` 时非空; `type:document` 时禁止 | `[F-v3-resource-example-invalid]` | 直接注入 `<examples>` |
| `path` | string | document 必填 | 无 | 文件存在且不逃逸 skill root | `[F-v3-resource-example-path-missing]` / `[F-v3-resource-example-path-invalid]` | runtime `read_example` 读取目标 |
| `summary` | string | document 必填 | inline 可选 | document 下 trim 后非空 | `[F-v3-resource-example-summary-missing]` | 扩展案例库目录说明 |
