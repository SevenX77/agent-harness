...
|---|---|---|
| **Skill Layout & File Grammar** | `[[KB-01-skill-anatomy]]` | Standard directory layout, `GRAPH.md` format, and phase three-name consistency rules. |
| **Dataflow, Schemas, & Blackboard** | `[[KB-02-io-dataflow]]` | Strict Draft 2020-12 schema rules, blackboard slicing/merging, and sequential overwrite authorization. |
| **Python Actions & Logic Purity** | `[[KB-03-logic-actions]]` | Local Python action signatures, read-only input purity constraints, and validator runtime contracts. |
| **LLM Agent Nodes & Prompts** | `[[KB-04-agent-nodes]]` | Agent prompt structures (role/goal/steps/protocols/examples), static mention reachability, and LLM roles. |
| **Subgraph Inclusion** | `[[KB-05-subgraph]]` | Subgraph relative paths, POSIX path rules, and parent-to-child schema mappings. |
| **Loops & Batch Iterations** | `[[KB-06-iterate]]` | Batch and loop iteration specifications, loop accumulators, and merge strategies. |
