...
| P2c-1 | 录 A1/A2 基线:主机名猜测的后果实录 | 🚧 待合并 | 100 条(4 协议 × 4 主机 × endpoint 名),把「猜」的结果钉死后再动它。**录出来的不是偶发猜错,是主机名压过用户声明的协议**:`anthropic_compatible` / `google_genai` 的 endpoint 只要 base_url 落在 `api.deepseek.com`,探测就发 OpenAI chat 请求;`openai_compatible` 的 endpoint 只要**名字**里含 deepseek,预算字段就从 `max_completion_tokens` 变 `max_tokens`。顺带记下:`test_provider_endpoint`/`test_provider_route` 这两个公开函数以 `test_` 开头,谁 import 谁就被 pytest 当测试收集——P2c-2 随搬迁改名为 `probe_*`,与 `probe_official_call_method` 一致 |
| P2c-2 | A1/A2 认领 call method,删掉主机名/名字猜测 | ⏳ 待办 | `endpoint_probe_backend` 改问 catalog(`call_method_ids_for_endpoint` → `provider_probe_backend_for_method`),deepseek 的两条规则补进 `host_overrides`(限定在各自协议内,跨协议改写在这张表里表达不出来);A2 的请求改由认领到的方法的方言渲染;两个 `test_*` 公开函数改名 `probe_*`。**已知一处必须同时拆开**:`provider_probe_backend_for_method` 现在把「方法不存在」和「这个方法没有官方探测」混成同一个 `Unknown official call method` 报错,于是 openrouter endpoint (唯一候选 `openrouter_anthropic_messages`,`official_probe: false`)一问 backend 就炸——而 endpoint 级可达性探测本来就不该被「有没有官方方法探测」卡住,它的 `provider_backend` 字段写的正是 `claude`。判据 = P2c-1 那 100 条基线的 diff **逐条能说出为什么变**,不许有说不清的行 |
