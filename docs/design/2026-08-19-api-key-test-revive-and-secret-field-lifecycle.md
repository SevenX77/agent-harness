...
- **要就地编辑真实密钥**,先按 Eye 揭示(S4 的显式动作),揭示后字段进入 `plaintext` 态,
  可正常编辑。
- **修订(2026-08-20,真机点验后)**:`plaintext` 态**聚焦即显真值**。首版把"聚焦不改变显示"
  当成对两种态通用的规则,漏掉了一件事——`plaintext` 的那把密钥客户端**已经握着**(Eye 取回过,
  或就是用户刚敲进去的),把它也按掩码处理并不多保护任何东西,却让下一次击键把掩码点并进密钥,
  保存被 D5 的边界挡回 422、界面上只表现为"改不动"。规则的边界是**这把密钥是不是要向服务器
  换取**:`stored` 要换,所以只有 Eye/Copy 能换;`plaintext` 不用换,聚焦显示它没有多披露一个
  字符。实证见真机点验第 7 条。
