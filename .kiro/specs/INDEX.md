...
| [`community-probe-catalog-service-phase2a/`](./community-probe-catalog-service-phase2a/) | Draft (Phase 2 / post-MVP1) | 社区探测目录服务 Phase 2a 免费档(三方审核定稿):serverless 门卫(不持写仓 token)+ 定时发布 Action(唯一写仓)· 客户端脱敏+白名单丢弃 opt-in 上传 · manifest/分片校验同步到 disposable cache。`studio-llm-remote-draft-catalog` 的二期后续 | [`docs/development/COMMUNITY_PROBE_CATALOG_SERVICE_DESIGN.md`](../../docs/development/COMMUNITY_PROBE_CATALOG_SERVICE_DESIGN.md) |
