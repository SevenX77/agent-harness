...
- **gateway 公共内核(③b)**：model group 分组 / **6 态标准投影** / **materialize 编排** / endpoint 标准化 + canonical id / Probe Knowledge Catalog（探测知识库）/ 批量探测策略 / 熔断持久化 → 归 `packages/graph-agent-gateway`，引用 [`../../graph-agent-gateway/mvp1/`](../../graph-agent-gateway/mvp1/)，**不在 studio 复制**。
