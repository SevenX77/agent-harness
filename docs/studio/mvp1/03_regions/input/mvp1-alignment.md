...
- **runtime import 冲突**: 同一 scope 下多个 candidate 能匹配同一 schema 字段时,不静默覆盖、不生成 binding;UI 显示冲突但 checkbox 仍可操作(checkbox 只负责改 md schema),lint/compile 返回 `STUDIO_RUNTIME_INPUT_CONFLICT`。这里的“挡住”只指 lint/compile/run/predict 不能产出通过结果,不锁 UI 编辑。
  - **冲突的范围与分级(Operator 2026-07-31 裁定,真机事故收敛)**: ①**范围 = schema 已声明字段**(`io.inputs.properties`);图没声明的字段即使多文件同名,也与本图无关,**不得产出任何诊断**。②**分级**:required 字段冲突 = `fatal`(跑不了);已声明但非 required 的字段冲突 = `warning`(该字段静默丢绑定,图仍可跑,但作者必须知道)。③**呈现 = 唯一诊断链**:冲突经 lint/compile 诊断集流到各表面,由 IO 面板的字段行诊断标记渲染;**IO 面板不得自建冲突 banner**(旧实现自造红条且谎称“lint/compile will fail”,已删)。事故:一个未声明字段的冲突让面板长期报红而 compile 正常通过,三个表面互相矛盾。
- **Predict/Run 输入供给的唯一权威(Operator 2026-07-31 裁定)**: 选中的 test input **覆盖**本次 payload;**未选中不是错误**——图的根输入由 runtime_config 的 import 绑定供给(呼应 `01_workflows/04_run-and-verify.md:35` PM 原话“input 和batch 都是节点配置问题, 和predict无关, predict和run就是按照配置来跑就行了”)。输入是否真的接齐,**只由后端预检裁决**并以 `STUDIO_RUNTIME_INPUT_MISSING` / `_CONFLICT` 出诊断;**前端不得在客户端自设“必须先选 test input”的闸**(旧实现在 `resolveRunInput` 直接 throw、请求根本不发出,与后端契约和本设计双重冲突,已删)。
- **嵌套寻址三层贯通(PM 2026-07-03 r5 核心)**: object 字段(如 `chapter`)在配置树里按层级展开,子字段(`chapter.aa_number`)**可独立勾选**;勾选一条子路径 = 在节点 io.inputs 里把 `chapter` 声明成 object 且其 `aa_number` 进 `required`(勾父级=声明整块 object 必需;勾子级=父级自动成为必需 object + 该子字段必需)。这要求**全链路认识嵌套路径,否则前端画的展开树是会撒谎的 UI(勾了没运行语义)**,故本轮三层同时改:① **engine** `StateMapper` 的 required 门禁(今名 `require_declared_inputs`)改**递归遍历 schema 树**(顶层与嵌套 `required` 同一套,缺失即 `[F-v3-runtime-state-mapping-failed]`,`field_path` 为点路径,与 output 侧 Draft2020 校验器统一;见 `docs/engine/mvp1/01-contract/03-compile-rules/mvp1-alignment.md` slice 行);② **studio backend** `canvas_data_gap.field_supply` 投影**递归展开**嵌套 properties,子路径作独立可寻址条目(带各自 producer/consumer,整块 object 的 producer 供其子路径);③ **前端** io-config / edge-static-inference 从「按顶层 key」改「按树遍历」(`fieldPathRows` 共享层),配置树可展开勾选。**输出侧不对称**:artifacts 由 engine writer 按**顶层 key**落盘(`artifact_manifest.write_manifest_artifacts` 只认顶层),故输出配置树里嵌套子字段**仅展示形状、不独立勾选**——一个 artifact 携带的是整块顶层字段(含其子字段),没有「只装 `segmentation_result.bb_number`」的引擎语义。
