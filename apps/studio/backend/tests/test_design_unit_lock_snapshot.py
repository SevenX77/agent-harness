...
INDEX_PATH = DOCS_ROOT / "DESIGN_UNITS_INDEX.md"
...
SNAPSHOT_REMEDIATION = (
    "if this is an intentional locked unit owner/state change, update "
    "docs/studio/mvp1/_design-unit-lock-snapshot.json with owner approval and keep it protected in "
    "docs/studio/mvp1/_audited-ready-hashes.json; otherwise restore DESIGN_UNITS_INDEX.md"
)
...
EXTERNAL_MODULE_PREFIXES = ("engine:", "gateway:")
EXTERNAL_MODULES = {"graph-agent-gateway"}
...
def _is_external_module(module: str) -> bool:
    return module.startswith(EXTERNAL_MODULE_PREFIXES) or module in EXTERNAL_MODULES
...
def _parse_markdown_table_rows(markdown: str) -> list[dict[str, str]]:
    ...
    for line in lines[header_index + 2 :]:
        ...
        if len(cells) != len(headers):
            raise AssertionError(f"Malformed DESIGN_UNITS_INDEX.md row has {len(cells)} cells, expected {len(headers)}: {line}")
    ...
    for raw_entry in spans.split(" · "):
        ...
        if "→" not in entry:
            raise AssertionError(
                f"{unit}: span entry must be atomic 'facet→`module`(role)': {entry}; remediation: "
                "split free text into a role-marked span entry"
            )
        if entry.count("→") != 1:
            raise AssertionError(
                f"{unit}: span entry contains nested arrows: {entry}; remediation: split nested references into "
                "separate 'facet→`module`(role)' entries"
            )
        ...
        if not match:
            raise AssertionError(
                ...
                "add an explicit leading role owner/消费/引/落点"
            )
...
def _canonicalize_design_units(index_path: Path) -> list[DesignUnitLockRecord]:
    ...
    for row in rows:
        ...
        has_external_refs = bool(external_refs)
        if external_binding == "none" and has_external_refs:
            raise AssertionError(
        ...
        if external_binding != "none" and not has_external_refs:
            raise AssertionError(
                f"{unit}: external-binding is {external_binding!r} but no engine/gateway '(引)' spans were found; "
...
def _load_snapshot(snapshot_path: Path) -> dict[str, Any]:
    assert snapshot_path.exists(), (
        "Missing Studio design unit lock snapshot: docs/studio/mvp1/_design-unit-lock-snapshot.json; "
        "remediation: generate it from the current locked DESIGN_UNITS_INDEX.md and add its SHA-256 to "
        "docs/studio/mvp1/_audited-ready-hashes.json"
    )
    data = json.loads(snapshot_path.read_text(encoding="utf-8"))
    assert isinstance(data, dict), "design unit lock snapshot must be a JSON object"
    assert data.get("schema_version") == 1, "design unit lock snapshot schema_version must be 1"

    units = data.get("units")
    assert isinstance(units, list), "design unit lock snapshot must contain a units list"
...
def _load_design_unit_exemptions() -> set[str]:
    ...
    exemptions = data.get("design_unit_lock_snapshot_exemptions", [])
    assert isinstance(exemptions, list), "design_unit_lock_snapshot_exemptions must be a list"

    exempt_units: set[str] = set()
    for index, exemption in enumerate(exemptions):
        assert isinstance(exemption, dict), f"design unit lock snapshot exemption #{index} must be a mapping"
        unit = exemption.get("unit")
        reason = exemption.get("reason")
        owner_approval = exemption.get("owner_approval")
        assert isinstance(unit, str) and unit, f"design unit lock snapshot exemption #{index} must include unit"
        assert isinstance(reason, str) and reason, f"design unit lock snapshot exemption #{index} must include reason"
        assert isinstance(owner_approval, str) and owner_approval, (
            f"design unit lock snapshot exemption #{index} must include owner_approval"
        )
    ...
    *,
    ...
    for unit, snapshot_record in sorted(snapshot_records.items()):
        if unit in exempt_units:
            continue
        if snapshot_record.owned_lock != "locked":
            continue
...
def test_design_unit_lock_snapshot_reports_locked_unit_drift_and_allows_new_drafts() -> None:
    snapshot_records = {
    ...
    assert any(
        "existing-locked" in violation and "owners" in violation and "alpha" in violation and "beta" in violation
...
def test_studio_design_unit_lock_snapshot_matches_current_index() -> None:
    snapshot = _load_snapshot(SNAPSHOT_PATH)
    current_records = _canonicalize_design_units(INDEX_PATH)
    snapshot_records = _snapshot_units(snapshot)
    exempt_units = _load_design_unit_exemptions()
    ...
    assert not violations, "Unapproved Studio design unit lock drift:\n" + "\n".join(violations)
